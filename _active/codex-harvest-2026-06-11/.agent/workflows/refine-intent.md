...
1. `semantic_libraries/antigravity/primitives/co-creative-launchpad-contract.md`
2. `directives/intent-pipeline.md`

Start from the current context and produce the Co-Creative Launchpad Packet
before asking questions. Same 4 stages, but start from current context:

1. **SCORE** — Rate current intent sharpness 1-5
2. **SHARPEN** — If Score ≤ 3, run DICE on missing dimensions only; otherwise ask only questions that change route, artifact, scope, risk, taste, or proof
