...
Before Phase 1, read `semantic_libraries/antigravity/primitives/source-to-skill-extraction.md`.
Also read `semantic_libraries/antigravity/primitives/skill-system-contract.md` when the source teaches orchestration, workflow chaining, OS design, or how multiple skills should work together.

Use it to classify the real build shape before creating files:
...
- new skill
- agent
- productized business asset

Do not proceed as a new standalone skill if the source is better handled as an expansion, companion, workflow, skill system, reference, companion OS layer, or semantic primitive.

### Phase 1: Source Acquisition

1. **If YouTube URL**: Build a video context ledger before extraction.
```bash
// turbo
python3 execution/video_context_ledger.py "<url>" --mode full
```

Use `extractions/video-context/<video-id>/` as the source package. Read `transcript.txt` as the clean human transcript and use `transcript_segments.json` plus `video-context-ledger.md` for timestamped evidence. This is required when visuals, slides, demos, screen recordings, ads, tutorials, charts, or creative references may affect extraction.

Verify package shape before deep extraction:
...
3. **Read the full source material**. Do NOT skim. Forge requires complete comprehension. Use the clean transcript for reading, timestamped segments for evidence, and the ledger for visual/OCR/uncertainty lanes. Preserve `uncertain_or_unavailable` rows in extraction notes and never treat inferred visual context as observed evidence.
...
4. **Semantic cold-start check**: Confirm at least one generated workflow or document can be used by a future agent without hidden explanation. If this fails, add a semantic primitive, example, or validation note before finalize.
