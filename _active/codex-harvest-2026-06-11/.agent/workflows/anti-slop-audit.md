...
- **Support Gate**: `.agents/skills/impeccable/SKILL.md` when the target is a frontend interface, HTML file, app route, or URL
...
### Step 0: Impeccable Deterministic Scan When Possible

If the design target is a file, directory, or URL, run:

```bash
npm run impeccable:detect -- [target]
```

Capture the structured findings and use them as evidence in the scorecard. If the scan cannot run, state why and continue with the human audit.

For qualitative refinement, load the relevant Impeccable reference:
- `critique` for UX and visual hierarchy
- `polish` for final craft
- `bolder` for generic or too-safe work
- `typeset` for font and hierarchy problems
- `layout` for spacing, rhythm, and composition problems
...
- Impeccable detector findings (if a scan target was available)
