...
Run the protocol tracker to show protocol lifecycle state:

```bash
python3 execution/protocol_tracker.py audit
```

**Review the output.** Flag any critical protocols showing as overdue:
...
If critical protocols are overdue, note them for the report. Dormant and cold/source-only protocols should be classified before any activation work.
