...
```bash
python /Users/farricecain/Codex\ Antigravity/execution/generate_image.py --aspect [RATIO] "[PROMPT]"
```

For image editing (modifying an existing image):
```bash
python /Users/farricecain/Codex\ Antigravity/execution/generate_image.py --edit [INPUT_PATH] "[EDIT_INSTRUCTIONS]"
```

For style transfer or character consistency (using prior image as reference):
```bash
python /Users/farricecain/Codex\ Antigravity/execution/generate_image.py --reference [REF_PATH] "[PROMPT]"
```
