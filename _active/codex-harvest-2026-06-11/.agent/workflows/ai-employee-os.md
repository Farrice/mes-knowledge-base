...
Read these before serious use:

1. `CODEX.md`
2. `semantic_libraries/antigravity/primitives/ai-employee-operating-contract.md`
3. `skills/fryderyk-wiatrowski-ai-employee-os/SKILL.md`
4. `skills/fryderyk-wiatrowski-ai-employee-os/workflows/ai-employee-os.md`
5. `extractions/video-context/ohKt066uFhg/evidence-map.md`
6. `extractions/video-context/ohKt066uFhg/uncertainty-report.md`
...
If no mode is provided, infer the mode. Use `--audit` for existing targets and `--design` for new roles.

## Operating Contract

| Field | Required Behavior |
|---|---|
| Source evidence | Cite the Viktor source package and keep OCR/visual limits explicit. |
| Objective | Produce an AI employee scorecard, contract, maps, guard, and first implementation sequence. |
| Components | This workflow, the root skill, semantic primitive, evidence package, supporting routes, and validation scripts. |
...
| Result surface | Rendered Conversation Document by default, with local source files only when implementing system artifacts; human-facing Markdown sources must open as readable documents, with metadata in sidecar JSON. |
| Context policy | Keep this command and semantic contract compact; load source package and detailed references on demand. |
...
python3 execution/routing_governor.py evaluate "[ai employee/system goal]"
...
- `/conde-agent-experience-design`: trust, progress states, command UX, and handoffs.
- `/24-assets-agent-system-design`: agent ownership, roster, and data flow.
...
2. **Evidence and target read**: load compact source package, contract, and target files.
...
10. **Implementation sequence**: give the smallest safe build, validation commands, and cold-start prompt.
...
The system must identify source evidence paths, component order, context/access risks, approval gates, validation checks, and first action without hidden chat context.
