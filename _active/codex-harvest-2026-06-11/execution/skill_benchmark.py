...
def _local_performance_history(skill_name: str, limit: int = 20) -> list:
    """Read local-first performance history for a skill."""
    ...
    for line in LOCAL_PERFORMANCE_LOG.read_text(encoding='utf-8').splitlines():
        ...
        try:
            ...
            continue
        if skill_name not in str(row.get('Skill') or ''):
            continue
        entries.append({
            'id': row.get('Local ID', ''),
            'url': row.get('Notion URL') or f"local://{row.get('Local ID', '')}",
            ...
            'User Rating': row.get('User Rating'),
            'Intent Alignment': row.get('Intent Alignment'),
            'Expert Standard': row.get('Expert Standard'),
            'Adversarial Resilience': row.get('Adversarial Resilience'),
            'Status': row.get('Status', ''),
            'Notes': row.get('Notes', ''),
            'Experiment Tag': row.get('Experiment Tag', ''),
            'Lane': row.get('Lane', ''),
            'Source Artifact': row.get('Source Artifact', ''),
            'Registration Source': row.get('Registration Source', ''),
            'Auto Confidence': row.get('Auto Confidence'),
            'Review State': row.get('Review State', ''),
        })
...
def get_performance_history(skill_name: str, limit: int = 20) -> list:
    """Pull performance history for a skill, using local-first evidence by default."""
...
def benchmark_skill(skill_name: str) -> dict:
    ...
    valid_workflows = {w['name'] for w in info['workflows']}
    workflow_scores = {}
