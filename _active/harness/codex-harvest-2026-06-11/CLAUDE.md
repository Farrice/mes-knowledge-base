...
This file provides legacy Claude Code (claude.ai/code) guidance for this repository. In Codex, active authority lives in `AGENTS.md` and `CODEX.md`; use this file as historical/source reference only unless a task explicitly asks for Claude-specific behavior or legacy comparison.

<!-- Legacy note: this file no longer drives Codex routing authority. Codex-native system intent should be represented in AGENTS.md, CODEX.md, .agent/workflows/, semantic_libraries/, and execution/ verifiers. -->
