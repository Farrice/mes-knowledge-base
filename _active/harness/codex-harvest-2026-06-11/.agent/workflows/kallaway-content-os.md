---
description: Full Kallaway content execution across strategy, production, hooks, storytelling, monetization, and source-backed content OS routing
---

# /kallaway-content-os - Kallaway Content Operating System

Load and execute `skills/kallaway-content-operating-system/workflows/content-os-orchestrator.md`.

## Required Context

Read:

1. `skills/kallaway-content-operating-system/SKILL.md`
2. `skills/kallaway-content-operating-system/references/source-evidence-map.md`
3. `skills/kallaway-content-operating-system/references/skill-system-contract.md`

Then load only the downstream Kallaway component files needed for the request.

## Output

Produce:

- route trace
- source evidence summary
- component chain
- handoff summaries
- first usable artifact
- validation and next-use route
