---
description: Create creative direction, design briefs, visual systems, product moments, and taste-audited design execution paths
---
...
```bash
/creative-design-agent [goal/context/source]
/creative-design-agent --deep [high-stakes or complex goal]
/creative-design-agent --council [multi-perspective goal]
```
...
1. `agents/creative-design-agent/AGENT.md`
2. `agents/creative-design-agent/memory/context.md`
3. `.agent/creative-design-agent-state.md` if present
...
5. **Run the Impeccable gate when UI or frontend craft is involved**: use `/impeccable shape`, `/impeccable critique`, `/impeccable polish`, `/impeccable typeset`, `/impeccable layout`, or `/impeccable bolder` as the support pass for taste, hierarchy, responsiveness, and anti-generic refinement.
...
```markdown
# Creative Design Agent Output: [Goal]
