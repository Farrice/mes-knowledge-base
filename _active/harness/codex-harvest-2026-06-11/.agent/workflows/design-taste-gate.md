...
| **Impeccable** | `.agents/skills/impeccable/` | Deterministic frontend craft, anti-pattern detection, and command-specific refinement |
...
When the target is an HTML file, app route, screenshot-backed page, or URL, load `.agents/skills/impeccable/SKILL.md` and run the most relevant Impeccable support pass:
- `critique` for overall UX and hierarchy
- `polish` for final production detail
- `typeset` for typography
- `layout` for spacing and composition
- `bolder` for bland or generic work
- `audit` when accessibility, performance, or responsive issues are part of the concern

If a file, directory, or URL can be scanned, run:

```bash
npm run impeccable:detect -- [target]
```

Treat detector findings as evidence inside the taste report, not as a replacement for the CEV and typography judgment.
...
- **`--impeccable [target]`**: Include Impeccable reference guidance and detector evidence when a scannable target is available.
