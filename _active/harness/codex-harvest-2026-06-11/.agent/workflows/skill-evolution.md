...
Also read `semantic_libraries/antigravity/primitives/goal-loop-maintenance-contract.md`.
`/skill-evolution` is mutation-capable, so it must not generate or apply a
variant unless the Goal Packet is complete.
...
Then create or confirm the Goal Packet:
...
If any field is missing, stop with a queue-only diagnosis and the missing
fields. Do not generate a variant.

### 0b. Evolution Council Preflight

Run the compact Evolution Council from
`goal-loop-maintenance-contract.md` before Step 1:

- Forager compresses logs, benchmark evidence, and target files.
- Architect proposes the smallest mutation.
- Skeptic names failure modes and overfitting risk.
- Implementer scopes touched files.
- Evaluator defines stop checks, no-regression checks, and wake-up checks.

Proceed only after the Evolution Council Verdict says the goal packet is
complete and the permitted side effect is approved.
...
Use the local-first candidate helper:

```bash
python3 execution/skill_evolution_candidates.py scan --write
```

Present candidates ranked by improvement potential. `ready` candidates can move into hypothesis selection; `watchlist` candidates need more evidence; `blocked` candidates must not be evolved until the blocker is cleared.
