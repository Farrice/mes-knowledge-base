...
from paths import BASE_PATH, BRAIN_DIR

# ---------------------------------------------------------------------------
# Paths
# ---------------------------------------------------------------------------
INDEX_JSON = BRAIN_DIR / "_index" / "conversations.json"
ORG_MANIFEST_JSON = BASE_PATH / "_system" / "organization" / "manifest.json"
...
def load_org_manifest() -> list:
    """Load the global organization manifest when available."""
    if not ORG_MANIFEST_JSON.exists():
        return []
    try:
        data = json.loads(ORG_MANIFEST_JSON.read_text(encoding="utf-8"))
    except Exception:
        return []
...
def search_org_entries(entries: list, query: str) -> list:
    """Search global organization entries by path, project, domain, and route metadata."""
    query_lower = query.lower()
...
def display_org_results(results: list, max_results: int = 12):
    """Display organization manifest matches."""
    if not results:
        return

    shown = results[:max_results]
    print(f"\n🗂️  Organization matches: {len(results)}" + (f" (showing top {max_results})" if len(results) > max_results else ""))
    print("=" * 70)
    for i, entry in enumerate(shown, 1):
        print(f"\n{i}. {entry.get('relative_path', entry.get('path', ''))}")
        print(f"   Domain: {entry.get('domain', 'unknown')}  |  Project: {entry.get('project') or 'unassigned'}")
        print(f"   Status: {entry.get('status', 'unknown')}  |  Type: {entry.get('artifact_type', 'unknown')}")
        if entry.get("destination"):
            print(f"   Route: {entry['destination']}")
    print("\n" + "=" * 70)
...
def main():
    ...
    if args.query:
        ...
        org_results = search_org_entries(org_entries, args.query)
    
    if args.domain:
        results = filter_domain(results, args.domain)
        org_results = [
            entry
            for entry in (org_results or org_entries)
            if args.domain.lower() in str(entry.get("domain", "")).lower()
        ]
    ...
    if args.json:
        print(
            json.dumps(
                {
                    "conversations": results[:args.max],
                    "organization": org_results[:args.max],
                },
                indent=2,
                default=str,
            )
        ...
        display_org_results(org_results, max_results=min(args.max, 12))
