...
LOCAL_MARKDOWN_PACKAGES = Path(__file__).resolve().parent.parent / ".tmp" / "python-packages"
if LOCAL_MARKDOWN_PACKAGES.exists():
    sys.path.insert(0, str(LOCAL_MARKDOWN_PACKAGES))
...
PLAIN_TEXT_FENCE_LANGUAGES = {"", "text", "plain", "plaintext"}


def is_plain_text_fence(fence_line):
    """Return True for fenced blocks that should render as client-copy text."""
    info = fence_line.strip()[3:].strip().lower()
    language = info.split()[0] if info else ""
    return language in PLAIN_TEXT_FENCE_LANGUAGES


def render_copy_block(lines):
    """Render plain text fence contents as native-looking Google Docs text."""
    paragraphs = []
    current = []

    for line in lines:
        if line.strip():
            current.append(html.escape(line.rstrip()))
        elif current:
...
def preprocess_markdown(md_text):
    ...
    for line in lines:
        ...
        if stripped.startswith('```'):
            if in_plain_text_block:
                processed.append(render_copy_block(plain_text_lines))
                processed.append('')
                in_plain_text_block = False
                plain_text_lines = []
                continue

            if not in_code_block and is_plain_text_fence(stripped):
                in_plain_text_block = True
                plain_text_lines = []
                continue
...
def set_pageless(doc_id):
    ...
    if result.returncode != 0 or "error" in combined_output:
        ...
        print(f"  ⚠ Pageless format failed: {error_text[:200]}")
    else:
...
def convert_and_upload(md_path, folder_id=None, name=None, pageless=True):
    ...
    # Write the short-lived import file beside the source so the gws upload
    # validator accepts the path in sandboxed Codex runs.
    with tempfile.NamedTemporaryFile(mode='w', suffix='.html', delete=False, encoding='utf-8', dir=str(md_path.parent)) as f:
