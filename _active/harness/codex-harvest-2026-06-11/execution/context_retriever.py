...
HARNESS_CONTEXT_DOCS = [
    ...
    "agents/operator-autopilot/AGENT.md",
...
def is_harness_diagnostic_query(text):
    """Detect control-plane/harness diagnostics so local contracts outrank broad skills."""
    ...
    return (has_context and has_failure) or broad_repair


def harness_context_chunks():
    """Return compact local control-plane chunks without rebuilding the skill index."""
    chunks = []
    for rel_path in HARNESS_CONTEXT_DOCS:
        path = ROOT / rel_path
        if not path.exists():
            continue
        try:
        ...
        chunks.append(
            {
                "source": rel_path,
                "expert": "antigravity-control-plane",
                "skill": "harness-runtime",
                "file_type": path.name,
                "section": "Control Plane Contract",
                "content": content[:6000],
                "word_count": word_count(content[:6000]),
            }
        )
    return chunks
...
def retrieve_context(task_intent, expert=None, top_k=5):
    ...
    harness_query = is_harness_diagnostic_query(task_intent)
    if harness_query and not expert:
        chunks = harness_context_chunks() + chunks
    ...
    for i, (chunk, doc_tf) in enumerate(zip(chunks, doc_tfs)):
        ...
        score += header_bonus
        if harness_query and chunk.get("source") in HARNESS_CONTEXT_DOCS:
            score += 1.0
