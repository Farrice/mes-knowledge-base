...
class HybridKnowledgeRetrieval:
    ...
    def _search_notion(self, query: str) -> List[KnowledgeSnippet]:
        """Search Notion through the project API wrapper with honest fallback."""
        try:
            try:
                from execution.notion_api import DB_IDS, NotionAPI
            except ModuleNotFoundError:
                from notion_api import DB_IDS, NotionAPI

            api = NotionAPI()
            snippets: List[KnowledgeSnippet] = []
            seen_urls: set[str] = set()

            def title_from_item(item: dict, preferred_prop: str = "") -> str:
                if preferred_prop:
                    prop = item.get("properties", {}).get(preferred_prop, {})
                ...
                if item.get("object") == "page" and "properties" in item:
                    for prop in item.get("properties", {}).values():
                        if prop.get("type") == "title" and prop.get("title"):
                            return prop["title"][0].get("plain_text", "Untitled")
                if item.get("object") == "database" and item.get("title"):
                    return item["title"][0].get("plain_text", "Untitled")
                return "Untitled"

            def add_snippet(item: dict, source: str, preferred_prop: str = "", score: float = 0.75):
                url = item.get("url", "")
                ...
                snippets.append(KnowledgeSnippet(
                    source=source,
                    content=f"{title_from_item(item, preferred_prop)} — {url}",
            ...
            for alias, title_prop in title_props.items():
                try:
                    db_result = api.query_database(
                        DB_IDS[alias],
                        filter={"property": title_prop, "title": {"contains": query}},
                        page_size=3,
                    )
                except Exception:
                    continue
                for page in db_result.get("results", []):
                    add_snippet(page, f"notion:{alias}", preferred_prop=title_prop, score=0.85)
                    if len(snippets) >= 5:
                        return snippets

            result = api.search(query, page_size=5)

            for item in result.get("results", []):
                object_type = item.get("object", "item")
                add_snippet(item, f"notion:{object_type}")
                if len(snippets) >= 5:
                    break
            ...
            return [KnowledgeSnippet(
                ...
                content=(
                    "Notion search unavailable in this execution context. "
                    f"Reason: {e}. Use local files or rerun in a network-enabled context."
                ),
                relevance_score=0.1,
                timestamp=datetime.now(timezone.utc),
            )]
    ...
    def _cache_result(self, query: str, scope: str, result: KnowledgeResult):
        ...
        if any(snippet.source.endswith(":unavailable") for snippet in result.results):
            return
