...
Usage:
    ...
    python execution/routing_intelligence.py misroute --request "make income" --wrong "generic-workflow" --correct "first-10k"
"""
import argparse
import json
import os
...
DATA_FILE = Path(os.getenv("ANTIGRAVITY_ROUTING_DATA_FILE", ROOT / ".agent" / "routing-intelligence.json"))
...
def log_misroute(
    ...
    rating: str = "negative",
    notes: str = "",
    session_id: str = "misroute",
    domain_detected: str = "routing",
) -> tuple[str, str]:
    """Log a bad route and its correction without requiring a routing_id."""
    clean_wrong = wrong_workflow.strip().lstrip("/")
    clean_correct = correct_workflow.strip().lstrip("/")
    routing_id = log_routing_decision(
        request_summary=request_summary,
        intent_score=1 if rating == "negative" else 2,
        domain_detected=domain_detected,
        experts_deployed=["antigravity-orchestrator"],
        tier_loaded=1,
        mode="output",
        workflow_used=clean_wrong,
        ensemble=False,
        compound_pairing="",
        session_id=session_id,
    )
    feedback_notes = f"Misroute: /{clean_wrong} should have routed to /{clean_correct}."
    if notes:
        feedback_notes = f"{feedback_notes} {notes}"
    feedback_id = log_feedback(
        routing_id=routing_id,
        rating=rating,
        session_id=session_id,
        correction=clean_correct,
...
def underperforming_routes(data: dict) -> str:
    """Routes with negative/mixed feedback and their corrections."""
    decisions = data.get("routing_decisions", [])
    feedback_log = data.get("feedback_log", [])

    flagged_fbs = [fb for fb in feedback_log if fb.get("rating") in {"negative", "mixed"}]
    if not flagged_fbs:
        return "No negative or mixed feedback recorded. Either everything's working well, or feedback isn't being captured yet.\n"

    # Map routing_id to decision
    rd_map = {rd["routing_id"]: rd for rd in decisions}

    lines = ["| Rating | Workflow | Expert(s) | Domain | Correction | Notes |",
             "|--------|----------|-----------|--------|------------|-------|"]
    for fb in flagged_fbs:
        rd = rd_map.get(fb["routing_id"], {})
        workflow = rd.get("workflow_used") or "unknown"
        experts = ", ".join(rd.get("experts_deployed", ["unknown"]))
        domain = rd.get("domain_detected", "unknown")
        correction = fb.get("correction") or "—"
        notes = fb.get("notes") or "—"
        lines.append(f"| {fb.get('rating', 'mixed')} | {workflow} | {experts} | {domain} | {correction} | {notes} |")
...
def _generate_suggestions(data: dict) -> List[str]:
    ...
    flagged = [fb for fb in feedback_log if fb.get("rating") in {"negative", "mixed"}]
    if flagged:
        latest = flagged[-1]
        correction = latest.get("correction") or "unspecified correction"
        suggestions.append(
            f"**Routing fix queued**: latest {latest.get('rating')} feedback points to `{correction}`. Review underperforming routes before approving router changes."
        )
...
def main():
    ...
    # misroute
    mr_p = subparsers.add_parser("misroute", help="Log a bad route and correction without looking up a routing ID")
    mr_p.add_argument("--request", required=True, help="Original request or query")
    mr_p.add_argument("--wrong", required=True, help="Wrong workflow that was selected")
    mr_p.add_argument("--correct", required=True, help="Workflow that should have been selected")
    mr_p.add_argument("--rating", default="negative", choices=["negative", "mixed"], help="Severity of the misroute")
    mr_p.add_argument("--session", default="misroute", help="Session/source label")
    mr_p.add_argument("--domain", default="routing", help="Detected domain label")
    mr_p.add_argument("--notes", default="", help="Free text notes")
    ...
    if args.command == "log":
        ...
        rid, fid = log_misroute(
            request_summary=args.request,
            wrong_workflow=args.wrong,
            correct_workflow=args.correct,
            rating=args.rating,
            notes=args.notes,
            session_id=args.session,
            domain_detected=args.domain,
        )
        print(f"routing_id={rid}")
        print(f"feedback_id={fid}")
