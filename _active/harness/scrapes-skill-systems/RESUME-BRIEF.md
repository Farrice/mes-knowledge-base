...
1. **Template pool for his brand** — `scrapes_brand.py check farrice --pool linkedin-carousel` says `render path: blocked` (no pool). Four refs are already staged in `brand_context/visual_refs/` (Premium Minimal carousel frames 01–04). Run `00-social-content` onboarding Phase 4.95 → 5 (one `ssc-template-builder` per ref; GPT Image est. $0.17/image, month $0.00/$15.00), approve in Template Studio. Until then a carousel for his brand stops at the check with a plain reason.
