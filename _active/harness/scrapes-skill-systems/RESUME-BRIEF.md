...
2. **Blind bar 01 verdict** — three takes sit as 1/2/3 in `JUDGING-SURFACE.html` (key: `JUDGING-KEY.json`, open after tapping). Which clears the bar, which is the exemplar, one change each. Named gap: photo zones carry evidence screenshots (Ad Library, product page, case page), not product photography (AG1 site bot-blocked; stock/AI refused). Then: publish AG1 by name or the composite (sends stay human). Content Studio on Take B: `http://127.0.0.1:59173/`.
