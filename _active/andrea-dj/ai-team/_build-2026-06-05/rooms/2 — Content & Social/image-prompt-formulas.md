...
*Daytime-locked prompt scaffolds tuned for Resonance's anchor aesthetic: editorial broadsheet left in actual sunlight. Every prompt below produces output that passes the 11pm test. Last updated: 2026-05-04. Status: canonical.*
...
The Resonance brand is photography-led. Real photographs of real moments are always the first choice — see `01-visual/photography-rules.md`. AI image generation is **tertiary use only** (textures for IG story backgrounds, mood-board internal references, content blocks where no real photo exists yet, pre-Event-#1 hero placeholders).

This file gives Andrea the prompt scaffolds to generate AI imagery that doesn't betray the brand. Every formula below has been pressure-tested against the photography rules. Every prompt produces output that:
...
4. Avoids identifiable AI faces (faces are forbidden in public Resonance use).
...
Every Resonance AI image prompt obeys the same skeleton:
...
| **Real daylight marker** | "real afternoon daylight" / "south-facing window light at 2pm" / "warm directional sunlight on a wood floor" / "overcast Chicago afternoon" / "sun through a curtain" |
| **Real room/location context** | "converted Chicago loft" / "a 1970s warehouse studio with exposed brick" / "wood floor, cream walls, Chicago window architecture" / "a Pilsen storefront at corner sunset" |
...
These tokens, in any combination, drag the output toward 11pm-coded or wellness-coded imagery. Banned across every Resonance prompt:
...
- **"golden hour"** *as a stylized filter* — produces faked late-afternoon warmth, not real daylight (real golden-hour-on-Chicago at 4pm in May is fine; "golden hour aesthetic" is not)
...
- **"north-facing window light"** (cooler, overcast Chicago register)
...
- **"Sandro Miller portrait sensibility"** (Chicago photographer; sometimes useful)
...
```
A mixed-race group of adults aged 30 to 40 dancing together in a converted Chicago
...
toward camera. Tight crop on hands and torsos. Warm cream walls, Chicago window
...
### Scene 3 — Andrea-Style DJ At Decks Daytime (Placeholder for Real Photography)
...
moment of work. Wood floor, cream-painted walls, Chicago loft architecture in
...
**Use for**: founder-portrait placeholder ONLY when no real Andrea photo exists (event #1 first edition). Replace with real photography immediately after Event #1.

**Critical rule**: Andrea is photographed by humans. AI Andrea is a placeholder, never a final image, never used in press.

---

### Scene 4 — Pilsen Sunset Corner (Chicago Atmosphere)
...
**Use for**: Chicago-anchor content blocks, the city-context image in a press one-sheeter, IG story location-establishing shot.
...
```
Two adults aged 30-40 standing close to each other in a converted Chicago loft,
...
blur on one body. Wood floor, cream walls, Chicago window architecture.
...
```
An empty hardwood floor in a converted Chicago loft, warm 2pm daylight pouring
...
### Scene 9 — Coffee Cup On A Window Sill (Nora's Register)
...
**Use for**: the Nora-profile-anchored content (essays about ICP), email newsletter that targets Profile #1 specifically.

---

### Scene 10 — A Record Store Aisle (Marcus's Register)
...
hand. Real Chicago record store interior — wood floor, cream walls, vinyl
visible in soft focus background. No flash, no filter, minimal grading.
--ar 4:5 --stylize 100
```

**Use for**: Profile #3 (Marcus) content, the "I have no idea how to do this anymore" moment essay, the quiet-man-targeted content piece.

---

### Scene 11 — A Therapist's Sunday Morning (Imani's Register)
...
**Use for**: Profile #2 (Imani) content, the "I don't have it in me to perform first-date energy" essay, the wellness-tourist-rejection content piece.
...
For Midjourney v6+ and Stable Diffusion based models, append a negative prompt to push against banned tokens. Resonance's negative prompt baseline:
...
a converted Chicago loft at 2pm on a Saturday. South-facing window light
...
brick walls, exposed-beam ceiling, Chicago industrial window architecture in
...
```
Late afternoon Chicago daylight pouring through a tall warehouse window onto
...
2. **Add specific Chicago context**. "Chicago loft" / "Pilsen storefront" / "Wicker Park warehouse" — geography pulls the model away from generic "club" defaults.
...
- **Press one-sheeter founder portrait**. Andrea is photographed by humans.
- **Press one-sheeter event imagery**. Real photography only.
- **Founder portraits of Andrea anywhere, ever**. AI Andrea is forbidden across the entire system.
- **Identifiable faces of attendees**. Faces are forbidden in AI imagery for public Resonance use, full stop.
...
2. **Real-room context named** — converted loft, Chicago window architecture, etc.?
