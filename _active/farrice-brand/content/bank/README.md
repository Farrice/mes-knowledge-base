...
## Rules
1. Every angle traces to a pillar; orphan angles get a `P?` tag and a weekly triage.
2. Nothing ships from here without the voice layer (BLEND) + prose gate + claim gate (if health-adjacent).
3. The bank is append-heavy, draw-down-light: if it grows past ~100 unused angles, prune bottom-up by staleness.
