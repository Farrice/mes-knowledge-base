...
Purpose: choose the canonical **L1 episodic-capture layer** with scored evidence, not vibes. Mirrors `_active/platform-bakeoff/PROTOCOL.md` — the decision rule is locked BEFORE any session is scored; post-hoc rationalization is the failure mode this file exists to prevent.
...
4. Run a scorecard (clone `_active/platform-bakeoff/scorecard.py`) to aggregate the weighted composite and apply the rule mechanically.
