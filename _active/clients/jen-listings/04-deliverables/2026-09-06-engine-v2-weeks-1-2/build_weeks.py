...
def post_manifest(p):
    ...
    if p["kind"] == "reel":
        ...
        # pre-rendered by another generator family member (the approved code-words package); files copied in, never re-rendered
        fmt = f"carousel · {len(p['files'])} slides · approved design, her photos + approved plates"
        hook = p.get("hook") or p["slides_text"][0]
        thumb = f"{p['id']}-1"
        slides = len(p["files"])
        photos = [pathlib.Path(f).stem for f in p["files"]]
    else:
