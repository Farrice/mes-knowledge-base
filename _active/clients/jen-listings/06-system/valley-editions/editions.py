...
def cover_exact(src, eyebrow, line_italic, line_upright, subline, handle="@_jiing", pos="50% 50%", scale=1.0, fill=None, size=166, stripe=True):
...
{f'<div class="abs" style="left:{int(W*0.271)}px;top:{int(H*0.919)}px;width:{int(W*0.648)}px;height:3px;background:{CREAM};opacity:.9;transform:rotate(6.94deg)"></div>' if stripe else ''}
...
def headline_size(text, cap, box_px, usable, em=0.58, lh=0.85):
...
def wash_d1():
    ...
    return (f'<div class="abs" style="inset:0;background:linear-gradient(180deg, rgba({WASH},.60) 0%, rgba({WASH},.55) 33%, '
            f'rgba({WASH},.55) 66%, rgba({WASH},.425) 100%)"></div>')
...
def fact_panel(left, eyebrow, big, small, top=None, w=None, h=None):
    ...
    return (f'<div class="abs" style="left:{left}px;top:{top}px;width:{w}px;height:{h}px;border-radius:30px;overflow:hidden;'
            ...
            f'<div class="abs" style="left:24px;right:24px;top:{h-64}px;{SANS}font-weight:300;font-size:18px;line-height:1.3;color:{WHITE}">{small}</div></div>')
...
def spot_exact(bg, masthead_text, headline, body, label, panel_l, panel_r, handle="@_jiing"):
    ...
    size = headline_size(headline, 166, int(H * (0.416 - 0.238)) - 8, int(W * 0.783))
    return (f'<div class="page">{bg}{wash_d1()}'
            f'<div class="abs" style="left:0;right:0;top:{int(H*0.072)}px;text-align:center;{SANS}font-weight:400;font-size:24px;line-height:1.4;letter-spacing:-.02em;color:{WHITE}">{masthead_text}</div>'
            ...
            f'<div class="abs" style="left:{int(W*0.206)}px;width:{int(W*0.554)}px;top:{int(H*0.50)}px;text-align:center;{SANS}font-weight:300;font-size:26px;line-height:1.4;color:{WHITE}">{body}</div>'
            f'{panel_l}{panel_r}'
            f'<div class="abs caps" style="left:0;right:0;top:{int(H*0.941)}px;text-align:center;font-size:17px;opacity:.9">{label}</div>'
            f'</div>')
...
def close_exact(bg, masthead_text, headline, close_line, hand_line, handle="@_jiing"):
    ...
    size = headline_size(headline, 166, int(H * (0.533 - 0.346)) - 8, int(W * 0.783))
    return (f'<div class="page">{bg}{wash_d1()}'
            f'<div class="abs" style="left:0;right:0;top:{int(H*0.072)}px;text-align:center;{SANS}font-weight:400;font-size:24px;line-height:1.4;letter-spacing:-.02em;color:{WHITE}">{masthead_text}</div>'
            ...
            f'<div class="abs" style="left:{int(W*0.18)}px;width:{int(W*0.64)}px;top:{int(H*0.63)}px;text-align:center;{SANS}font-weight:300;font-size:27px;line-height:1.45;color:{WHITE}">{close_line}</div>'
            f'<div class="abs" style="left:0;right:0;top:{int(H*0.74)}px;text-align:center;{HAND}font-size:52px;color:{WHITE}">{hand_line}</div>'
            f'{smiley(int(W*0.451) + 8, int(H*0.864), 100)}'
            f'</div>')
...
def cover_guide_exact(src, eyebrow, headline, body, handle="@_jiing", pos="50% 50%", scale=1.0, fill=None):
    ...
    img = extended(src, pos, scale, fill) if fill else photo(src, pos, scale)
...
def frames():
    ...
    return [
        ("Main", "01 · cover", cover_exact(PH / "jen-headshot-studio.jpg", MAST, "$869K", "in Tarzana.",
            ...
            pos="100% 100%", scale=0.55, fill="#8d8878", stripe=False)),
        ("Frame02Laidrey", "02 · place", spot_exact(plate_bg("A", "laidrey coffee roasters, 18600 ventura blvd, 7am light, empty sidewalk, no people"),
            MAST, "Laidrey,<br>7am.",
            ...
            plate_inset("B", "espresso bar, no faces", INSET_L), plate_inset("C", "one cup on a table, ventura blvd soft behind", INSET_R))),
        ...
        ("Frame04Buys", "04 · what $869K buys", spot_exact(plate_bg("D", "a valley-floor street in tarzana 91335, single-story homes, morning, no people, no house numbers"),
            ...
            plate_inset("E", "a front porch step, a house key on it, no hands", INSET_L),
        ...
        ("SheetGuide", "sheet · D4 city guide", cover_guide_exact(PH / "jen-headshot-studio.jpg", "the valley &#183; edition 01", "$869K in<br>Tarzana.",
            "three homes at one number. a 7am coffee on ventura.", pos="100% 100%", scale=0.55, fill="#8d8878")),
