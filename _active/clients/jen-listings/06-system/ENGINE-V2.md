...
| 4 | AMPLIFY | Caption: ONE pen (Alyssa hook-reframe + Luke Iha hooks) makes one sharpening pass; ONE check (Jen-as-herself) reverts any line she would not say. **On-screen line (Farrice 2026-09-09): a hook room of four pens (Kallaway, Forsyth, Harding, Luke Iha) each writing five beat pairs against the specimen, one integrator culling against the bar, eight survivors on a blind judging surface with the specimen seeded unlabeled, his tap decides.** Pens write, never critique (the Sept 2 "expert soup" was critics) | `AMPLIFY: one pen on the caption · hook room: 4 pens · 20 pairs · 8 to the surface · Jen check reverted N` |
...
| 7 | DELIVER | `jen_os_page_thumbs.py` + `jen_os_page.py` → the Valley OS page; files to Drive `04 · ready to post / week-of-*`; posting checklist (story slide, collab tag, first comment, routing); Feedback Triad | `DELIVER: URL · Drive folder · triad pending` |
...
§4's shares are now set by `CONTENT-MIX.md` (Attract 45 / Connect 25 / Position 15 / Convert 15 while a listing is active) until her own numbers replace them. The hook rule applies to every district: the post opens on her or the reader's situation; the number or the house is beat 2. Approved specimen (the writing floor for every district): `04-deliverables/social-content/2026-09-07-content-recovery/outputs/jen-asking-again.md` (Farrice 2026-09-07: "This is the register").
