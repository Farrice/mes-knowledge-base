...
def main():
    ...
    for old_name in ['identity-overview.png','04-close-white.png','04-close-navy.png']:
        old_path=deliver/old_name
        if old_path.exists(): old_path.unlink()
    for name in ['01-remodel-date','02-in-unit-laundry','03-private-showing']:
        shutil.copy2(OUT/f'{name}.png',deliver/f'{name}.png')
    shutil.copy2(OUT/'04-close-navy.png',deliver/'04-next-listing.png')
    names=['01-remodel-date','02-in-unit-laundry','03-private-showing','04-close-navy']; thumb=(360,450)
    ...
    comparison=Image.new('RGB',(830,570),(225,223,217)); dc=ImageDraw.Draw(comparison)
    for i,name in enumerate(['04-close-white','04-close-navy']):
        im=Image.open(OUT/f'{name}.png').convert('RGB'); im.thumbnail(thumb); x=25+i*405
        comparison.paste(im,(x,60)); dc.text((x,28),name,fill=(22,58,82))
    comparison.save(HERE/'close-comparison.jpg',quality=94)
