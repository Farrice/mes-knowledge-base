# G2 Voice + AI-Tell Scan — Resonance Brand Operating System v1
...
The BOS is Andrea-fluent at the example level and AI-leaky at the connective-tissue level. The voice document, the hook library sample fills, and the manifesto excerpts read like Andrea. The prose around them — the section openers, the "why-it-matters" paragraphs, the meta-commentary — does not. The single most propagating leak is em-dash overuse, including inside the spine sentence itself, which then cascades into every doc that paste-mounts the spine. The structural-tell ban list is mostly honored at the example level (Andrea's banned-moves discipline is real), but the explanatory layer that surrounds the examples leans on the same moves the doc tells writers to avoid.
...
Why it's a problem: the bible is the second-most-pasted doc and the closest thing to "this is what Andrea sounds like" reference material for AI sessions. The body's em-dash density teaches a register that contradicts the voice doc's "≤2 per piece" rule.
...
File: `00-foundation/02-icp-master.md` lines 165, 167-168 (Imani's USE list): "*Soft Sunday* / *slow morning*" and Marcus's USE list line 244: "*I want someone I can be tired around.*" — these are intentional ICP voice samples and OK.
Real issue: Marcus avatar line 211 — "He has therapy every other Tuesday at 8am. Plays no video games. Cooks. Calls his grandmother. Has a journal." is a triple-beat with a fourth landing — fine, this is anaphora done right.
False alarm withdrawn. ICP master holds.

**H8. The "It's not X. It's Y." reveal pattern inside the file that bans it.**
File: `00-foundation/02-icp-master.md` line 196 (Imani test sentences):
...
Suggested fix: rewrite Imani's magnet to: *"Not a sober community. A room that doesn't need alcohol to come alive. You're invited as a person, not as a holder."* The frame-then-sharpen pattern lands the same job without triggering the contrast-reveal tic.

**H9. "Here is the part nobody talks about" ghost — softer variant.**
File: `00-foundation/01-brand-bible.md` line 78 — "**Enemy 4 — The algorithm pattern (the silent one)**" header + body line 80 "The apps are not the manifesto's rallying cry because every dating brand attacks the apps. Resonance attacks them at a deeper level..."
...
Why consider: the term is doing real work, but it's becoming the BOS's own catchphrase. When a phrase appears 12+ times across a brand system, it shifts from diagnostic to decoration. Imani would clock this.
...
Pattern: "Daytime. Sober. Curated. Chicago." appears verbatim or near-verbatim in 6+ places. The crystallized-phrase pattern is the brand signature, but the same exact crystallization repeated verbatim weakens the move.
...
Files: content pillars, hook library meta-commentary. "The brand survives on Andrea-recognition." "The brand is built so..." "The brand has no commentary mandate."
...
Suggested fix: in body copy, prefer "Resonance" or "the room" over "the brand." "The brand" is a strategy-doc word, not an Andrea word.
...
Files: all foundation docs. Italics serve double duty (Andrea-quoted manifesto lines + author emphasis) which weakens the convention. Voice doc Section 5 final bullet bans italicized mid-paragraph aphorisms, but author emphasis italics are doing similar work in places.
...
Same issue as L2 — internal-author register that should never show up in Andrea-facing or audience-facing surfaces.

**L4. "Real" is becoming an over-promise word.**
Files: ICP master Marcus profile, hook library Singles Reality pillar. "Real" appears 40+ times across the system. Every brand claims "real." When it appears that often, the word goes transparent.
...
The hook library sample fills land more often than not. "I curate the music myself, beat to beat. The rule is whether the song bends the room toward someone you wouldn't have noticed" (line 281) is exactly the register Andrea claims. "He brought a record. He didn't tell me until the end of the night" (line 138) does the show-first work the voice doc demands. The Marcus avatar sequence in the ICP master (record store scene, lines 213-215) is the sharpest piece of show-don't-tell in the system. These are the proofs that the voice exists and can be reproduced.
...
2. **Rewrite Imani's magnet line in `02-icp-master.md` line 196** to remove the "It's not X. It's Y." reveal. The line is a public-facing example of the structural move the BOS teaches AI sessions to avoid; either the example or the rule has to give, and the rule is more valuable.
