# 04-content-os — index

> The ongoing content engine. CONTENT-OS.md (master) + content-creation-system + social-playbook + exemplars/ + starter-content-batch.md (CANONICAL post text) + prompt-kits/prompt-sets + voice-gate + content-team-os.
>
> _Part of the LinkedIn Business OS. Front door: `../04-deliverables/00-CONTROL-TOWER.md`. Folders mirror the Control Tower's §1–§6. Reorganized 2026-07-01._

## Contents
- `CONTENT-OS.md`
- `CONTENT-QUALITY-BENCHMARK-AND-RECIPE.md`
- `REVIEW-clean-posts.md`
- `ai-boom-Post1-ELEVATED.md`
- `ai-boom-content-package.md`
- `authority-flywheel-format-finder-onepager.md`
- `content-creation-system.md`
- `prompt-kit-audit.md`
- `sky-format-farrice.md`
- `social-playbook.md`
- `starter-content-batch.md`
- `voice-gate.md`

### Subfolders
- `arcs/` (4 files)
- `content-team-os/` (16 files)
- `exemplars/` (5 files)
- `mini-briefs/` (1 files)
- `novelty-engine-flagship/` (5 files)
- `posts/` (1 files)
- `prompt-kits/` (13 files)
- `prompt-sets/` (5 files)
- `voice-captures/` (1 files)
