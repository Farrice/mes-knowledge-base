---
status: parked
---

# Jen Santulan — Foundation / BOS

## Purpose
Repositioning brief, messaging spine, and Business Operating System work. The LIVE per-listing engine is a separate project: `_active/jen-listings/` (which carries its own CLAUDE.md).

## Map
- `assets/`
- `deliverables/`
- `foundation/`
- `production/`
- `recon/`

## Filing
New artifacts go in the canonical subfolder at creation time (`directives/artifact-placement.md`), never loose at the project root. Moving anything: `python3 execution/project_filer.py plan --project "<abs dir>"` — never bare `mv`.
