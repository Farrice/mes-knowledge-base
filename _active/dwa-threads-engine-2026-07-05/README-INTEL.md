...
Executive brief and index for the second intel wave (files 10-17). Built on real scraped social data (Reddit, TikTok, YouTube, IG) captured 2026-07-05 via Apify — total spend ~$0.74. QA verdict: **SHIP_WITH_FIXES** (full detail: `17-intel-qa.md`).
...
| File | What it is |
|---|---|
| `10-voice-of-customer.md` | Real Reddit/TikTok VoC pull — validates + corrects the ICP |
| `11-competitor-inventory.md` | ~15 named TikTok/IG competitors, credential/mechanism audit |
| `12-trending-bestpractices.md` | Format/hook patterns from 60+60 real TikTok videos |
| `13-postpurchase-sentiment.md` | What happens after people buy DWA/DBA/UBC — real reviews/threads |
| `14-strategy-fortification.md` | Whitespace bet re-tested against real data, self-flags the Threads-native gap |
| `15-product-ladder-pairings.md` | Offer ladder mapped to the corrected ICP and confirmed whitespace |
| `16-standalone-product-concept.md` | "The 14-Day Ship Sprint" $47 product spec + demo-sell-build pre-sale gate |
| `17-intel-qa.md` | Adversarial QA verdict — compliance leaks, validation gaps, risk ranking |
...
2. **Write to the corrected ICP, not the original one.** Lean into the mom-to-mom resale reality and the bimodal skeptic/credulous split when drafting Threads copy — don't let "Mark, 33" override what the real data showed. Route through `/copy-engine` or `/ghostwrite` with `10-voice-of-customer.md` loaded.
...
- **Trustpilot bot-walled** — could not scrape post-purchase review sentiment from Trustpilot directly; `13-postpurchase-sentiment.md` relies on Reddit/forum threads instead.
