...
`$ANTIGRAVITY_ROOT`. Stop at a human checkpoint for global
...
Verifier phrases: Mutation requires a complete goal packet, Evolution Council Verdict, baseline, search set, measurable stop condition, turn cap, proof artifact, and no-regression check. Permitted side effects must be local, reversible, and inside `$ANTIGRAVITY_ROOT`. Stop at a human checkpoint for global mirrors, external actions, broad archive/delete, destructive cleanup, new dependencies, failed validation, or scope expansion. Do not mutate Mission unless `verify_mission_activation_contract.py` fails and the user explicitly approves Mission repair. Route repair, drift-audit, and broken-system language to `/system-audit` or `/autopilot`.
