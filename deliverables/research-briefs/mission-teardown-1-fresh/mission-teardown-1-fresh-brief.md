...
```
python3 execution/handoff_store.py resume teardown-1-fresh
```
   touches: .agent/handoffs/2026-08-07-teardown-1-fresh.md
...
```
python3 execution/pulse_actions.py park teardown-1-fresh --reason "<one line>"
```
...
```
python3 execution/pulse_actions.py kill teardown-1-fresh --reason "<one line>"
```
...
SHIPPED IN WINDOW:
  - 2026-08-07 · Three named-brand teardowns (Create Wellness, Hilma, BPN) at Take A standard — full analysis + LinkedIn post + DM + verification ledger + adversarial pass each;
...
Fresh-pen packet run; receipts pulled live via Playwright/WebFetch ($0); prose gate 2.0/10 on shipping prose; repaired a concurrent-session directory rename mid-run (accept-repair-dedupe) | Factual Grounding: 9 | Verific
...
## the record
- **Teardown #1 — fresh pen, v1.3 card + atlas + conversion spine** [VERIFIED] — 3 teardowns shipped (Create Wellness, Hilma, BPN) — analysis + LinkedIn post + DM + verification ledger + adversarial pass each; 3 Briefing Room briefs w/ share variants; 10 brand-system cards via new execution/teardown_cards.py; $0 run. Ve (missions.jsonl · done)

## swings to
- [HANDOFF] Stored handoff (source of resume) — .agent/handoffs/2026-08-07-teardown-1-fresh.md
...
- `.agent/handoffs/2026-08-07-teardown-1-fresh.md` — playbook · Resume here
