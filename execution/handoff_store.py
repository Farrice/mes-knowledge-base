...
def _newest_temp_by_thread(thread):
    """Codex control-plane identity match (2026-08-01): filename-derived slug OR
    frontmatter-declared thread must match — identity is never chosen by recency."""
    cands = _temp_candidates()
    if not cands:
...
def cmd_save(args) -> int:
    ...
    if args.from_temp:
        if getattr(args, "thread", None):
            # Codex control-plane path: exact thread identity, never recency
            src, reason = _newest_temp_by_thread(args.thread)
            if src is None:
                print(f"ERROR: {reason} in temp ({tempfile.gettempdir()})", file=sys.stderr)
                return 1
            print(f"from-temp: {src} ({reason})",
            ...
            if len(fresh) > 1:
                ...
                print("or narrow with --slug <thread-slug> / --thread <thread>.", file=sys.stderr)
                return 1
            src = cands[0]
