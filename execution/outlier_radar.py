...
EVIDENCE_CLASSES = ("PRIVATE_OUTCOME", "OWNED_PROXY", "PUBLIC_PROXY")
DATA_MATURITY_STATES = ("UNDECLARED", "COLD_START", "HYBRID", "OWNED_LEARNING")
COHORT_ROLES = ("UNCLASSIFIED", "TOPIC_COHORT", "FORMAT_ONLY", "EXCLUDE")
SIGNAL_HYGIENE_STATES = ("PASS", "REVIEW", "REJECT")
ENGAGEMENT_FLOOR = 0.02
...
def data_maturity_state(owned_corpus_size: int | None) -> str:
    """Declare when first-party evidence should begin replacing public proxies."""
    if owned_corpus_size is None:
        return "UNDECLARED"
    if owned_corpus_size < 10:
        return "COLD_START"
    if owned_corpus_size < 20:
        return "HYBRID"
    return "OWNED_LEARNING"


def apply_signal_hygiene(record: dict[str, Any]) -> None:
    """Apply Kallaway's 2% engagement intake floor when metrics are available."""
    likes = record.get("likes")
    comments = record.get("comments")
    views = record.get("views")
    if views and likes is not None and comments is not None:
        rate = (likes + comments) / views
        record["engagement_rate"] = round(rate, 6)
        if rate < ENGAGEMENT_FLOOR:
            record["signal_hygiene"] = "REJECT"
            record["rejection_reasons"] = ["engagement_below_2_percent"]
        else:
            record["signal_hygiene"] = "PASS"
            record["rejection_reasons"] = []
        return
    record["engagement_rate"] = None
    record["signal_hygiene"] = "REVIEW"
    record["rejection_reasons"] = ["engagement_unavailable"]
...
def validate_pack(pack: dict[str, Any]) -> list[str]:
    ...
    if pack.get("pack_version") != PACK_VERSION:
        problems.append(f"pack_version must be {PACK_VERSION}: {pack.get('pack_version')!r}")
    if pack.get("evidence_class") not in EVIDENCE_CLASSES:
        problems.append(f"evidence_class invalid: {pack.get('evidence_class')!r}")
    if pack.get("data_maturity_state") not in DATA_MATURITY_STATES:
        problems.append(f"data_maturity_state invalid: {pack.get('data_maturity_state')!r}")
    owned_corpus_size = pack.get("owned_corpus_size")
    if owned_corpus_size is not None and (not isinstance(owned_corpus_size, int) or owned_corpus_size < 0):
        problems.append(f"owned_corpus_size invalid: {owned_corpus_size!r}")
    ...
    if not isinstance(ranked, list):
        ...
        for index, record in enumerate(ranked):
            ...
            if record.get("evidence_class") not in EVIDENCE_CLASSES:
                problems.append(f"ranked_videos[{index}].evidence_class invalid")
            if record.get("cohort_role") not in COHORT_ROLES:
                problems.append(f"ranked_videos[{index}].cohort_role invalid")
            if record.get("signal_hygiene") not in SIGNAL_HYGIENE_STATES:
                problems.append(f"ranked_videos[{index}].signal_hygiene invalid")
            if not isinstance(record.get("rejection_reasons"), list):
                problems.append(f"ranked_videos[{index}].rejection_reasons is not a list")
...
def cmd_refresh(args: argparse.Namespace) -> int:
    ...
    pack = {
        ...
        "evidence_class": "PUBLIC_PROXY",
        "owned_corpus_size": args.owned_corpus_size,
        "data_maturity_state": data_maturity_state(args.owned_corpus_size),
        "channels": [
            {
                "channel_id": row["channel_id"],
                "handle": "@" + row["handle"],
...
def parse_args() -> argparse.Namespace:
    ...
    refresh.add_argument(
        "--owned-corpus-size", type=int, default=None,
        help="Published first-party post count; declares COLD_START (<10), HYBRID (10-19), or OWNED_LEARNING (20+).",
    )
