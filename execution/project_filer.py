...
def apply_plan(plan: dict[str, Any], receipt_path: Path | None) -> dict[str, Any]:
    ...
    for item in move_items:
        ...
        if not source.exists():
            skipped.append({"path": str(source), "reason": "source missing"})
            continue
        ...
        revert_lines.append(
            f'mkdir -p "{source.parent}" && mv -n "{destination}" "{source}"'
        )
        append_move_ledger({
            "action": "project-file",
            "moved_at": utc_now(),
