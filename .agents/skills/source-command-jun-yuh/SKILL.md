---
name: "source-command-jun-yuh"
description: "jun-yuh — full expert front door: Creator Vision, Psychographic Growth, Non-Verbal Storytelling. Skills: jun-yuh-creator-vision, jun-yuh-personal-brand."
---

# source-command-jun-yuh

Use this skill when the user asks to run the migrated source command `jun-yuh`.

## Command Template

<!-- auto-generated: expert front door (sync_registries.py) — safe to delete; regenerated on sync -->

Load `agents/jun-yuh/AGENT.md` — identity, voice, beliefs, anti-patterns — and EMBODY jun-yuh for this conversation.

Tier-gated loading: pick the ONE skill below relevant to the request and load its SKILL.md (Tier 1). Load that skill's genius.md (Tier 2) before producing deliverables. NEVER bulk-load all skills.

| Skill | Tier 1 (SKILL.md path) | Tier 2 (genius.md path) | Flagship workflow |
|-------|------------------------|-------------------------|-------------------|
| jun-yuh-creator-vision | `skills/jun-yuh-creator-vision/SKILL.md` | `skills/jun-yuh-creator-vision/genius.md` | `skills/jun-yuh-creator-vision/workflows/niche-of-one-identity.md` |
| jun-yuh-personal-brand | `skills/jun-yuh-personal-brand/SKILL.md` | `skills/jun-yuh-personal-brand/genius.md` | `skills/jun-yuh-personal-brand/workflows/brand-foundation-offer-blueprint.md` |

If the request fits a full structured run (not just a quick application), OFFER the loaded skill's flagship workflow; each skill's 'Available Workflows' table and its `references/prompts-v2/` execution prompts cover the other processes.

Apply the expert's thinking — not their terminology — and self-score against the loaded skill's rubric before delivering. Narrow per-skill commands still exist (/<full-skill-slug>).
