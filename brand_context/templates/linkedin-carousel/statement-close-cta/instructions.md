...
- **HEADLINE** — the closing statement / insight
  - bbox: 8% 25% 90% 30% (left anchor matches masthead/footer at 8%; width extends to 90% — right
    margin ~2% — so the width-bound autosize has legal room without the box ever reading as glued to
    the edge, since no rendered line actually reaches past ~90% in practice)
  - style: display token (104px @ reference canvas ceiling, autosize shrinks to fit width down to
    ~99px in practice), line-height 1.0, letter-spacing -0.025em, sentence case, Helvetica Neue 700,
    paper (#FAFAF8) on ink, left-aligned, `white-space:nowrap` per line (the `<br>` breaks are hard —
    each line never re-wraps, it shrinks to fit instead); supports `<br>` for deliberate line breaks
    and `<mark>` for a single emphasis word
