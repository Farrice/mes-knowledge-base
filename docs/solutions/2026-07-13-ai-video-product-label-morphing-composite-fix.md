...
1. **Never let the model render the product.** Composite the real product PNG over the generated footage. Added a manifest-driven `overlays[]` track to the Remotion ad pipeline (`_active/clients/trendscale-trial/03-video-samples/remotion-pipeline/src/Ad.tsx`): path, start/end seconds, x/y/width percent, accent glow, fade-in. Glow doubles as an edge mask over the covered region.
