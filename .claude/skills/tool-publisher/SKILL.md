...
## Step 2.7: First Comment (LinkedIn)

**Only run this step when the target platform is `linkedin`.**

After the user confirms the publish in Step 2, ask:

```
LinkedIn penalizes reach for posts that include links in the body.
A standard workaround is to post the link (or CTA, hashtags, sources)
as the first comment, which Zernio posts automatically right after publishing.

Do you want to add a first comment to this post?
(Paste the text, or press Enter to skip)
```

- **If the user provides text:** store it as `first_comment`. Pass it to the publish step:
  - **REST path:** add `--first-comment "<text>"` to the `publish_rest.py` invocation.
  - **MCP path:** include `"firstComment": "<text>"` inside `platformSpecificData` when calling `posts_publish_now` / `posts_create`.
- **If the user skips / leaves blank:** publish normally without a first comment.

This step is optional. Do not block the publish if the user does not want a first comment.
...
- `--first-comment "text"` — post this text as the first comment right after publishing (Step 2.7). Falls back to `post.yaml` `first_comment:` if not passed. LinkedIn only in practice.
