...
Options:
  ...
  --first-comment TEXT      Text posted as the first comment immediately after
                            publishing. Falls back to post.yaml first_comment:.
                            LinkedIn: use this for links/CTAs/hashtags so they
                            don't penalize reach on the main post body.
...
def http(method: str, url: str, headers: dict | None = None, body: bytes | dict | list | None = None,
    ...
    try:
        ...
        # connect-time failure / DNS / refused — e.reason may itself be a timeout.
        reason = getattr(e, "reason", e)
        if isinstance(reason, (TimeoutError, socket.timeout)):
            fail(f"Zernio request timed out after {timeout}s — check your connection and try again")
        fail(f"network error reaching Zernio: {reason}")
    except (TimeoutError, socket.timeout):
        # READ timeout during resp.read() raises socket.timeout (TimeoutError in 3.10+)
        # and is NOT wrapped in URLError — without this it escaped as a raw traceback
        # (AIOS-139 Addendum 9 #5). Surface a clean, user-facing message instead.
        fail(f"Zernio request timed out after {timeout}s — check your connection and try again")
    except ssl.SSLError as e:
        fail(f"secure connection to Zernio failed: {e}")
    except OSError as e:
        fail(f"network error reaching Zernio: {e}")
...
def main() -> None:
    ...
    ap.add_argument("--first-comment", dest="first_comment",
                    help="Text to post as the first comment after publishing. "
                         "Falls back to post.yaml first_comment: field. "
                         "LinkedIn: use this for links, CTAs, or hashtags to avoid reach penalties.")
    ap.add_argument("--draft", action="store_true",
                    help="Create the post as a DRAFT (omit publishNow/scheduledFor). "
                         "Takes precedence over --schedule-for.")
    ...
    first_comment = args.first_comment or meta.get("first_comment") or None
    ...
    if args.pdf:
        ...
        psd["documentTitle"] = title
    if first_comment:
        psd["firstComment"] = first_comment
    if psd:
        platform_block["platformSpecificData"] = psd
    ...
    if args.draft:
        # Draft = omit BOTH publishNow and scheduledFor (Zernio defaults to draft).
        pass
