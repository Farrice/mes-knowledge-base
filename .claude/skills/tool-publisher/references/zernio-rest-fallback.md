...
### LinkedIn with first comment (links / CTAs / hashtags)

LinkedIn penalizes reach when links appear in the post body. Put them in the first comment instead — Zernio posts it automatically right after publishing.

```json
{
  "content": "Caption text (no links here)",
  "platforms": [{
    "platform": "linkedin",
    "accountId": "...",
    "platformSpecificData": {
      "firstComment": "Full article: https://example.com\n\n#tag1 #tag2"
    }
  }],
  "publishNow": true
}
...
`firstComment` and `documentTitle` can coexist in the same `platformSpecificData` object when publishing a PDF carousel with a first comment.

### LinkedIn carousel as individual images (up to 9)
```json
{
  "mediaItems": [
...
| `first_comment:` (optional string) | `platforms[].platformSpecificData.firstComment` |
...
> **Content Studio automates this (AIOS-139 Addendum 8 #2).** When you publish through Content Studio (`/post`), `content_studio.py`'s `_persist_publish_result` already patches `post.yaml` (`status: published` + the `publish:` block) AND appends the `{output_base}/publish-log.md` row on success — you do NOT do it by hand. The manual steps below apply only to a raw `publish_rest.py` invocation outside Content Studio.

The MCP flow auto-updates `post.yaml` with the publish result. With raw REST you do it manually:
