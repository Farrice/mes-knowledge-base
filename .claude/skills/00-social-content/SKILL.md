...
description: >
  ...
  (A) finished text → images only; (B) YouTube/video URL → post + images;
  (C) topic or idea → trending research → post + images;
  (D) no input / "from my sources" → scrape LinkedIn + YouTube → post + images;
  (E) existing post → repurpose for another platform;
  (F) article/blog/non-video URL → screenshot → extract → post + images;
  (G) local video/audio file → WhisperX transcribe → post + images.
  v2.0.0 delegates the visual layer (Visual Inventory + Slide Planning + Image Source Resolution)
  to a sub-agent `ssc-designer`, and writes the pipeline log per-run inside `{date}/{slug}/`
  instead of a shared `{date}/logs/` file.
  ...
  Repurposing triggers (→ mkt-content-repurposing): "adapt for", "repurpose for",
...
| **0.7 (onboarding)** | **orq** | **TEMPLATE STUDIO REVIEW — after template-building you MUST IMMEDIATELY launch the local-server Studio in template mode (`content_studio.py {brand_context}/templates/{pool} --mode template`, background, auto-open). This is an UNCONDITIONAL imperative, the ONLY allowed next action — NEVER ask "Quer que eu lance o Studio?" / "Want me to launch the Studio?" (asking instead of launching is a HARD ERROR). The user walks the pool, compares ref↔render, reads the builder's Template Card rationale, edits + `/approve`; block until the user approves/closes; THEN run the `≥1 status:ready` gate** | not first run (no template-building); user opts out via `--no-preview` |
...
| **7.5** | **orq** | **CONTENT STUDIO — launch the local-server editor (`content_studio.py`, background) which serves the live-HTML editor + auto-opens the browser; edits are live (WYSIWYG), the bake is implicit on Download/Publish; block until user approves** | `chosen_format == "text"`; user opts out via `--no-preview` |
| **7.6** | **orq** | **APPLY-BACK (fallback only) — Content Studio's `/apply` makes rebake live in-editor; this file-based path stays as the fallback when the server can't run (`render_template.py --tweaks`, NO AI)** | server unavailable AND user exported `tweaks.json` |
...
11. **Phase 7.5 Content Studio is blocking AND script-only.** After `ssc-image-generator` returns PNGs, launch the editor by calling `content_studio.py <run_folder>` via `Bash` **in the background** (`run_in_background: true`) — that's the ONLY allowed action. **Never use `Write` to author the preview HTML inline.** `content_studio.py` is a thin stdlib `http.server` wrapper that serves the editor `preview_editor.py` generates (introspected per-slide controls, LinkedIn mock, live-HTML slides), picks a free port, and **auto-opens the browser** — the user runs no command. In the browser the user edits text + layout **live (WYSIWYG)** — there is **no Apply button** (removed in FASE 5); the bake is **implicit** (on Download / Publish, `/apply` runs `render_template.py --tweaks` server-side and swaps in the real PNG — pixel-parity, NO AI). State persists to the run folder (`tweaks.json` + `comments.json` + `caption.md`) so re-opening restores the session. This kills the old export-file / paste-back loop. If the script doesn't cover your case, EXTEND THE SCRIPT, don't bypass it. Print the URL the server prints. Wait for explicit user approval before Phase 8. **Stop-handler:** if the server process exits with code 0 (the user clicked Close Studio), treat it as a deliberate close — do NOT relaunch. Only relaunch on a non-zero exit (crash). Relaunching on a clean exit causes an infinite loop.

    **Fallback (server can't run):** if `content_studio.py` fails to start (locked-down env, no free port), fall back to the static path — `preview_editor.py <run_folder>` emits a self-contained `preview/editor.html` the user double-clicks; they then **Export tweaks.json** and you run the file-based **Phase 7.6 apply-back**: drop `tweaks.json` into the run folder and rebake ONLY the affected slides (`render_template.py --tweaks tweaks.json` per slide) — same deterministic, no-AI overrides layer. With an empty/absent `tweaks.json` the output is byte-identical to the original bake (additive by default). Comment pins are preview-only annotations and never enter the bake or `tweaks.json`. For a pure read-only, non-edit preview (no editing needed at all), `preview_carousel.py` remains the static baked-PNG LinkedIn-mock fallback.

11b. **Onboarding Template Studio review is blocking AND script-only (mirrors Rule 11 for TEMPLATES, not the post).** **🚨 The post-build Studio launch is an UNCONDITIONAL IMPERATIVE you MUST execute — never a question. 🚨** The instant template-building completes during onboarding (`references/onboarding.md` Phase 6.7) and BEFORE the `≥1 status:ready` gate, you MUST IMMEDIATELY launch the Template Studio in **template mode** on the brand pool by calling `content_studio.py {brand_context}/templates/{pool} --mode template` via `Bash` **in the background** (`run_in_background: true`) — that is the **ONLY allowed next action**. You **MUST NOT** ask the user *"Quer que eu lance o Studio?"* / *"Want me to launch the Studio?"* — **asking for permission to launch instead of launching is a HARD ERROR.** The launch is automatic and NOT subject to the user's approval; the user approves the *templates inside* the Studio (via `/approve`), not the act of opening it. After launching, print the URL, tell the user to review/approve, and block on their approval. The only thing that suppresses the launch is the up-front `--no-preview` flag — never a chat question. **❌ Anti-pattern (HARD ERROR):** ending onboarding with a question like *"Próximo passo natural: abrir o Template Studio… Quer que eu lance agora?"* — the launch is automatic and not subject to the user's permission; if you are about to ask, STOP and call the `Bash` launch instead. **Never use `Write` to author the preview/editor HTML inline.** The pool dir is the launch target: `content_studio.py --mode template` accepts a pool dir directly (resolves the manifest, walks every `status: ready`/`approved` template, auto-picks a free port + auto-opens the browser — the user runs no command). The conference panel shows each template's ref↔render compare side-by-side PLUS the builder's **Template Card rationale** (`instructions.md`: form · per-block treatment + why · edit_mode · extraction) as review notes/improvement suggestions next to the compare. The `/approve` seam persists tweaks (`render_template.py --tweaks`, NO AI) and marks the template `approved` in the manifest. Print the URL the server prints and wait for explicit user approval/close before running the gate. Honor `--no-preview` (skip the launch, go straight to the gate). **Stop-handler:** if the server process exits with code 0 (the user clicked Close Studio), treat it as a deliberate close — do NOT relaunch. Only relaunch on a non-zero exit (crash). Relaunching on a clean exit causes an infinite loop. **Fallback (server can't start):** do NOT write inline HTML — surface the manifest's per-template previews (`brand_context/templates/{pool}/_preview/*.png` and each template's `preview.png`) for static by-eye review and confirm in chat; `preview_editor.py <template_dir>` is the per-template static editor fallback if a single template needs editing.
