...
A major upgrade to the whole post-generation experience.

### Added
- **Content Studio** — an interactive visual editor to fine-tune a generated post (text, images, layers) before publishing, including a "Magic Layer" that turns a flat image into editable pieces.
- Edit images directly with AI from inside the editor.

### Improved
- Smarter choices about what becomes baked-in artwork vs. editable text, so posts match the reference and stay editable where it matters.
- Broad generation-quality gains: stronger headlines, faithful brand styling, correct fonts, and cleaner image composition.
- A clearer, guided first-run setup.
