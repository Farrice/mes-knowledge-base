...
| `FULL_AI` | AI generates the whole slide | Text baked **into** the AI image | Yes, including text fidelity | Strictly gated for short-headline social-design archetypes (≤8 words, ≤60 chars, slide 1). **Exception: the `dense-infographic` archetype uses FULL_AI to bake all N×M labels verbatim — no headline cap, no HTML overlay.** |
...
**Exception — `dense-infographic`:** the `dense-infographic` archetype is EXEMPT from the headline ≤8-word/≤60-char cap and the no-body-text rule. It uses FULL_AI unconditionally and bakes ~20 dense labels verbatim across N ∈ 2–4 cards (each with M ∈ 2–4 bullets), N logo tiles, and N captions, staying balanced (N=2 → balanced two-column composition). The cap remains in force for all other archetypes.
