...
today. Styles are purely additive — a brand opts in via the POST-BUILD style grouping
(onboarding Phase 5.2, `references/template-authoring/shared/analyze-templates.md`): the
orchestrator proposes groupings from the finished previews, the user approves via
`AskUserQuestion`, and only then is `styles.json` written. Single-style brands never get one.
