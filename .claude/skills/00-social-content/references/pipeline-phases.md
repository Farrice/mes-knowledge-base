...
| Phase 7.5 editor ready | `→ Phase 7.5 — Interactive editor ready at {abs path} · edit live or type 'approve'.` |
| Phase 7.6 apply-back done | `→ Phase 7.6 — Rebaked {n} slide(s) from tweaks.json (no AI) · re-previewing.` |
...
2. **Detect available image providers — PREVIEW ONLY, never the binding gate (read .env every run).**

   ⚠️ **The Phase 1 read is a non-binding preview, not a cached verdict.** It only seeds the UI hint about which providers *look* configured at boot. NEVER let it dictate the whole run: a key pasted mid-run (between Phase 1 and the point it is used) MUST take effect. Each phase that actually needs a key **re-reads `.env` fresh at the point of use** and acts on that fresh read — never on this Phase 1 snapshot. This generalizes the per-request key read already done for Zernio publish (key-at-click, Addendum 7) and the project-root `.env` resolution (Addendum 9) to the ENTIRE pipeline: trending-research, image-gen, decompose, and publish each probe `.env` again when they run. If you ever find yourself acting on "no keys present" decided at Phase 1, that is the bug — re-read.
   ...
   Build the preview `available_providers` list:
   - `["openai", "gemini"]` if both pass
   - `["openai"]` or `["gemini"]` if only one
   - `[]` if neither

   This list is a preview only. The value passed to `ssc-image-generator` in Phase 7 is re-derived from a FRESH `.env` read at that point (see Phase 7), so it reflects keys added after this boot. The image-gen agent picks per slide using documented criteria when both available; forces the only one when one available; falls back to TEMPLATE-only rendering (no AI image generation) when neither.
...
- `available_providers` *(REQUIRED — re-read `.env` FRESH here, do NOT reuse the Phase 1 snapshot)* — immediately before spawning the image-gen, re-resolve `.env` (same `env_file` path, same validation as Phase 1 step 2) and rebuild the list: `["openai", "gemini"]` / `["openai"]` / `["gemini"]` / `[]`. This is the binding read — a key the user pasted after boot is honored here even though Phase 1 saw none. The image-gen agent picks per slide using documented criteria when both available; forces the only one when one available; downgrades HYBRID_AI / FULL_AI / HYBRID_FROM_REAL to TEMPLATE (or HYBRID_REAL if a real source resolved) when the list is empty.
...
- Slide N/M [role]:
    ...
    - Template: {pool}/{template_id} ({cover | body | cta})
...
## Phase 7.5: INTERACTIVE EDITOR *(orchestrator — blocking, between Phase 7 and Phase 8)*

**Purpose:** show the user how the post will actually look on LinkedIn AND let them refine it — text, layout, per-zone position/size/opacity/colour — live, BEFORE saving the final `post.yaml`. Catches layout/text issues at the cheapest possible moment AND lets the user fix them in place (no re-prompt) at the same checkpoint.

**Skipped when:** `chosen_format == "text"`, or user passed `--no-preview` flag.

### HARD-WON RULE — NEVER write HTML inline for Phase 7.5

**The ONLY allowed action in Phase 7.5 is calling `preview_editor.py` via `Bash`.**

- DO NOT use the `Write` tool to author `editor.html`, `index.html`, or any LinkedIn-mock / editor markup yourself
- DO NOT "supplement" the script's output with extra inline HTML
- DO NOT write a custom preview because you think the layout is missing something — if it's missing something, **fix the script** (`preview_editor.py`) and re-run. The script is the canonical source.
- DO NOT generate the HTML as a fallback because the script fails — surface the error to the user and stop

The script accepts a run folder and resolves everything it needs: each slide's `template.html` + `## Slots`, `brand_context`, `caption.md`, the brand logo for the avatar. It introspects each slide's template and **generates the control panel per zone type** — there is nothing the orchestrator knows that the script can't read. If you reach for `Write` here, stop and call the script instead.

**Self-improvement on script gap:** if you genuinely hit a case the script doesn't cover (a custom platform mock, a new zone type), extend `preview_editor.py` with a flag/branch — never write inline HTML. The next run benefits.

`preview_carousel.py` remains as the read-only fallback (static baked-PNG LinkedIn mock) for the non-edit path or if `preview_editor.py` can't render a particular run; the editor supersedes it as the default carousel preview.

### Steps

> **Content Studio (local-server) is the primary launch path (Addendum 3, AIOS-139).** `content_studio.py` is a thin stdlib `http.server` wrapper around the same editor `preview_editor.py` generates — it adds a live rebake (`/apply`), disk-persisted state (`/save` + `/load`), auto-port + auto-open. No new dependency; the editor UI is served as-is. The static `preview_editor.py` path below is the **fallback** when the server can't run.

1. Launch **Content Studio** in the background:
   `python .claude/skills/00-social-content/scripts/content-studio/content_studio.py {output_base}/{date}/{slug}/` (run with `run_in_background: true` — the server blocks while serving). It:
   - Reuses `preview_editor.build_editor_html` to serve the **live-HTML editor** (introspected per-slide controls, LinkedIn swipe-carousel mock, comment pins) **as-is** — no rebuild
   - Picks a **free port** (cross-OS via `socket`), starts the server, and **auto-opens the browser** (`webbrowser`) — the user runs no command
   - Exposes endpoints: **`POST /apply`** (rebake affected slides via `render_template.py --tweaks` → returns the real baked PNG live, **no AI**, pixel-parity), **`POST /save`** / **`GET /load`** (persist + restore `tweaks.json` + `comments.json` in the run folder), **`POST /ai-edit`** ("Edit with GPT/Gemini" on image layers — buttons appear only for providers whose `OPENAI_API_KEY` / `GEMINI_API_KEY` is present in the project `.env`; the result previews in a modal and Apply reuses the Replace-image `imgSrc` path), `GET /healthz`
   - Prints the URL (`http://127.0.0.1:<port>/`)

   **Fallback (server unavailable):** call `preview_editor.py {output_base}/{date}/{slug}/` which writes a self-contained `{slug}/preview/editor.html` static file the user double-clicks; edits are exported as `tweaks.json` and applied via the file-based Phase 7.6.

2. **Relay to the user.** Print the URL (or the static path on fallback) and add one sentence: *"Content Studio just opened — drag between slides, edit text/layout live (changes apply instantly — no Apply button), drop comment pins; I bake your edits in place automatically when you Download or Publish. Type 'approve' to ship."* (Fallback wording: *"...click **Export tweaks.json** and send me the file."*)

3. **WAIT for explicit user response.** Do not call Phase 8 automatically.

4. **Handle the response:**
   - "approve" / "go" / "ok" / "save" → proceed to Phase 8 (no tweaks = output identical to the original bake; Content Studio's implicit bake already rebaked any edits in place on Download/Publish)
   - (Fallback path) user sends a `tweaks.json` → **go to Phase 7.6 (apply-back)**
   - A bigger structural change (new slide, arc rewrite, palette swap) that the editor can't express → **route back through the canonical emit — this is IMPERATIVE, not a menu of options**: single slide → re-spawn `ssc-image-generator` for that slide; multi-slide/arc → `ssc-designer` then `ssc-image-generator`; color/font (cosmetic) → adjust `inferred_palette` / `inferred_typography` and re-render via `render_template.py --tweaks`, then re-run `preview_editor.py`. **There is no third path.** The orchestrator NEVER hand-authors or copies a patched `template.html` / asset (no `_patches/<x>/`, no project-root scratch, no derived dir) to "fix it itself" — every re-render goes through `render_template.py` (`--emit-edit-slide` / `--tweaks`, which guarantees the `_shared/` sibling) or a re-spawn. `render_template.py` enforces this: a re-render whose template origin is non-canonical (no resolvable `_shared/`, or under `_patches/`) HARD-FAILS the bake (`guard_canonical_template_origin`). See the extended hard rule in `references/onboarding.md` ("the orchestrator NEVER authors a template artifact").
   - **Defect that the legitimate path can't resolve → STOP and REPORT, do not improvise.** If you hit a template defect (e.g. a template has no slot for the content, or routes content through the AI prompt instead of HTML slots) that neither `--tweaks` nor a re-spawn fixes, the ONLY valid move is to **fail explicitly**: log `FAILED — upstream template defect` to `pipeline-log.md`, flag `needs-template-fix` / `needs-user-decision`, and surface it to the user. Do NOT hand-patch a template to "save the run". This generalizes the image-gen's text-verification stop (`agents/ssc-image-generator.md` — "stop, log FAILED, flag needs-user-decision. Do NOT silently present an unverified slide") from text-in-image to ANY template defect at the orchestrator level.
   - "redo from scratch" → ask what to change in briefing, restart from Phase 5.0

5. Log to `pipeline-log.md`:
   ```
   ## Phase 7.5 — INTERACTIVE EDITOR [done]
   - Decision: generated editor at {path}
   - User feedback: {approve | exported tweaks.json | structural edit | rejected}
   - Iterations: {n}
   - Elapsed: {s}
   ```
...
The pre-v2.0.1 flow went Phase 7 → Phase 8 → "here's a markdown summary, want to publish?". The user had to visually parse 6 PNGs in their file explorer one by one. The editor puts the post in one LinkedIn-shaped page so the user evaluates it AS A POST — and now also refines it in place (move a card 20px, fix a word, shrink a headline) without paying for a full re-prompt-and-render cycle.

---

## Phase 7.6: APPLY-BACK *(orchestrator — rebake-only, NO AI; runs only if the user exported tweaks)*

**Purpose:** turn the user's exported `tweaks.json` into final pixels by re-rendering ONLY the affected slides through the same `render_template.py` path the bake uses — deterministic, no AI image regeneration.

**Skipped when:** the user approved without exporting tweaks (the original bake is already final — additive-by-default).

### Steps

1. Save the user-provided `tweaks.json` into the run folder (`{output_base}/{date}/{slug}/tweaks.json`). It carries `global` overrides (accent / fonts / masthead) + one object per slide keyed by **stable zone handle** (`data-slot`) with the per-zone CSS knobs (`x`/`y`/`w`/`h`/`fontSize`/`opacity`/`tilt`/`color`/`scale`/`radius`/stroke/`text`).

2. For each slide that has a non-empty tweaks entry, rebake:
   ```
   python .claude/skills/viz-image-gen/scripts/render_template.py \
     --template {slide template.html} --data {slide data} \
     --tweaks tweaks.json --slide {slide_id} --out {slug}/slide-{n}.png
   ```
   - `render_template.py` consumes `tweaks.json` as an overrides layer on top of the base data and re-renders via headless Chromium. **No OpenAI / image-gen call.**
   - With an empty/absent `tweaks.json` the output is **byte-identical** to the original bake (purely additive — RNDR-04 parity).

3. Re-run `preview_editor.py` on the run folder so the user sees the rebaked slides. The applied PNG matches what the live preview showed (parity).

4. Loop back to Phase 7.5 step 3 (wait for approval). When approved, proceed to Phase 8.

5. Log to `pipeline-log.md`:
   ```
   ## Phase 7.6 — APPLY-BACK [done]
   - Tweaked slides: {list}
   - Rebake: render_template --tweaks (no AI)
   - Elapsed: {s}
   ```

**Comment pins are preview-only** — they live in `localStorage`, never enter `tweaks.json` or the bake, and are not part of apply-back.

---

## Onboarding Phase 6.7: TEMPLATE STUDIO REVIEW *(orchestrator — blocking, onboarding-only; parallel to Phase 7.5 but for TEMPLATES)*

**Purpose:** after the builder produces templates during onboarding (`references/onboarding.md` Phase 5–6) and auto-marks them `status: ready`, the USER has not yet seen them. This phase launches the **Template Studio** so the user walks the brand pool, compares each template's reference ↔ render side-by-side, edits on the canvas, reads the builder's reasoning, and **approves** — BEFORE the `≥1 status:ready` proceed-gate acts. This is the template analogue of the Phase 7.5 post editor; same launch mechanics, different `--mode` and target.

**Skipped when:** not first run (no template-building happens), or the user passed `--no-preview`.

### HARD-WON RULE — NEVER write HTML inline (same as Phase 7.5)

**The ONLY allowed action is calling `content_studio.py --mode template` via `Bash`.** Do NOT use `Write` to author any editor/preview/conference HTML. If the script doesn't cover a case, extend the script — never bypass it. The fallback when the server can't start is the manifest's static previews (below), never inline HTML.

### Steps

1. Launch **Template Studio** in template mode against the brand pool, in the background:
   `python .claude/skills/00-social-content/scripts/content-studio/content_studio.py {brand_context}/templates/{pool} --mode template` (run with `run_in_background: true` — the server blocks while serving). It:
   - Accepts the **pool dir** directly — `_resolve_pool_templates` reads `{pool}/manifest.json` and returns every `status: ready`/`approved` template in manifest order (the "Studio operates on the brand_context pool directly" decision, 2026-06-09). It opens the first as the active editing template; the main-screen ‹ › arrows step through the pool (`/select-template` + reload).
   - Picks a **free port**, starts the stdlib `http.server`, and **auto-opens the browser** (`webbrowser`) — the user runs no command.
   - Serves the **conference panel** (pool-walk): per active template a side-by-side **Reference | Render** compare via `POST /compare-images` (returns ref + render data-URIs), PLUS the builder's **Template Card rationale** read from that template's `instructions.md` (form · per-block treatment + why · edit_mode · extraction) as collapsible "Builder notes — why this template" review/improvement context next to the compare.
   - Exposes endpoints: `GET /healthz`, `GET /pool-templates` (ordered `{id, status, approved}` + active id), `POST /compare-images` (`{template_id}` → `{ref, render, notes?}`), `POST /select-template` (switch active template), `POST /apply` (rebake the active template via `render_template.py --tweaks`, NO AI), `POST /approve` (`{template_id, tweaks}` → persists the user's tweaks into that template's own dir + marks it `approved` in the manifest), `POST /save` / `GET /load`.
   - Prints the URL (`http://127.0.0.1:<port>/`).

   **Fallback (server unavailable — locked-down env / no free port):** do NOT write inline HTML. Surface the manifest's per-template previews — `brand_context/templates/{pool}/_preview/*.png` (gallery) and each template's `preview.png` — for a static by-eye review and have the user confirm in chat. For editing a single template, `preview_editor.py <template_dir>` is the static per-template editor fallback (same Export `tweaks.json` → `render_template.py --tweaks` apply-back path).

2. **Relay to the user.** Print the URL and one sentence: *"Template Studio just opened — for each template, compare its reference vs render (use the ‹ › arrows to step through the pool), edit text/layout live, read the builder's notes on why it was shaped this way, and click Approve. Type 'approve' when you're done."*

3. **WAIT for explicit user response.** Do not run the `≥1 status:ready` hard rule automatically.

4. **Handle the response:**
   - "approve" / "done" / "ok" → the per-template `/approve` calls already persisted tweaks + marked approved; run the `≥1 status:ready` hard rule (`references/onboarding.md`) and proceed.
   - A template needs a structural rebuild (not expressible by canvas tweaks) → **the ONLY valid move is to re-spawn `ssc-template-builder`** for that template's ref (same spawn contract as `onboarding.md` Phase 5) with feedback, then relaunch the Studio. The orchestrator NEVER hand-authors / copies a patched `template.html` (no `_patches/<x>/`, no out-of-pool dir) to fix it itself — that loses the pool's `_shared/` sibling and breaks brand-font injection. Cosmetic tweaks go through `render_template.py --tweaks` (the Studio's `/apply`); structural goes through a re-spawn. No third path; see the extended hard rule in `references/onboarding.md`.
   - **Defect a re-spawn can't fix → STOP and REPORT.** If a template defect is not resolvable through `--tweaks` or a re-spawn, fail explicitly: log `FAILED — upstream template defect`, flag `needs-template-fix` / `needs-user-decision`, surface to the user. Do NOT improvise a hand-patched template to proceed.

5. Confirm to the user: *"Templates reviewed ✓ ({N} approved)"* then continue to the proceed-gate.

### Why this matters

Without this, onboarding builds templates, the builder's auto quality-gate marks them `status: ready`, and the orchestrator proceeds straight to content generation — the user NEVER sees the front to validate/edit/approve the generated templates (confirmed by a real run; `onboarding.md` only checked `≥1 ready`, and the only Studio launch was Phase 7.5 for the POST). This phase closes that gap: the template-mode Studio + `/approve` seam were already built and validated (AIOS-139); this wires the launch into the pipeline.
...
1. **Review + iterate visually** — this is **Content Studio**, already opened at Phase 7.5. If the user closed it, relaunch the same editor (NOT the retired `viz-image-gen/preview/server.py`):
   ```
   👀 Review + edit in Content Studio (opens in your browser, auto-detects this run):
      python .claude/skills/00-social-content/scripts/content-studio/content_studio.py {output_base}/{date}/{slug}/
      → edit caption + slide text/layout live, edit/decompose images, Publish with Zernio
   ```
   > The old `viz-image-gen/preview/server.py` is **retired** — Content Studio (Phase 7.5) supersedes it. **One Content Studio runs per run folder** (it takes a single run-folder argument; don't start two servers on the same folder).

2. **Publish**: from inside Content Studio (**Publish with Zernio**), or `/tool-publisher`.
