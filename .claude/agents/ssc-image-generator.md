...
Backward compatibility: `HYBRID` is treated as `HYBRID_AI`. `FULL_AI` (text-baked-into-image) now executes natively for the `dense-infographic` archetype (`ai_style == "dense-infographic"`, the cover-framework-diagram) — AI composes the scene AND bakes ~20 labels verbatim, no HTML overlay. For all OTHER archetypes, a legacy `FULL_AI` value is still treated as `HYBRID_AI` and logged (text-baking is intentionally gated to dense-infographic).
...
|---|---|---|
| `ai_style == "dense-infographic"` | **GPT** | Text fidelity — bakes ~20 dense labels at ~95%+ accuracy (2026 A3 bake-off); GPT is mandatory for this archetype's text fidelity |
| `render_mode == "FULL_AI"` (any archetype) | **GPT** | Text fidelity — GPT bakes ~20 dense labels at ~95% accuracy (2026). Gemini 3 Pro Image improved to ~70-85% but GPT remains preferred for dense text-baking; the absolute "NEVER Gemini" no longer holds for shorter strings. |
...
Normalize legacy render-mode names: `HYBRID` → `HYBRID_AI`, `AI` → `FULL_AI`. `FULL_AI` falls back to the `HYBRID_AI` execution path **EXCEPT** when `ai_style == "dense-infographic"` (the cover-framework-diagram archetype), which runs the real FULL_AI dense-infographic execution variant (Step 8, Path D below). The only allowed FULL_AI->HYBRID_AI override for dense-infographic is a true sanitization failure on the headline — never the ≤8-word/≤60-char cap (that cap does not apply to this archetype).
...
7. **Determine render_mode upgrade** by consulting the `Render Mode × Archetype Decision Table` in `style-social-design.md`. Find the row for `selected_archetype_id`:
   ...
   - **If `ai_style == "dense-infographic"`:** set `render_mode = FULL_AI` unconditionally — skip the ≤8-word/≤60-char cap and the body-empty check entirely (this archetype bakes the full dense label set). Headline sanitization and the brand_name-not-a-literal rule still apply. Route execution to Path D (dense-infographic FULL_AI variant) in Step 8. Log: `dense-infographic FULL_AI — cap bypassed`.
...
Before falling through to Step 6's archetype-driven prompt construction, check whether the slide's template declares its prompt explicitly. Templates built by `ssc-template-builder` since the AI Composition Boundary convention (see `.claude/skills/00-social-content/references/template-authoring/shared/template-conventions.md > AI image zones`) embed one `[ai-image-zone:N]…[/ai-image-zone]` block in `template.html` per AI image slot.
...
The per-post prompt is built in three parts, **identity → style → delta** (the run-07 fix — the brand grade is no longer poured whole into every slide): the brand **identity** (palette / accent / grain, from `brand_style_source`) gives the feed unity; this template's **style** (medium / lighting / treatment, from the block's `image_style`) gives the piece its look; the **delta** (the subject the designer fills) is last. See "How to consume each block" step 3.
...
1. **Parse fields** with simple regex (format documented in `template-conventions.md`):
   ...
   - `identity_input:` — (optional, the `scene-restyle-with-real-face` ROTA) a SECOND `--input-image` carrying the IDENTITY (a brand headshot resolved to a real path, e.g. `brand_context/visual-identity/headshots/simon-pic.jpg`). When present, pass it as a second `--input-image` AFTER `ref_input` (ref = scene/medium, identity = the person). Absent → single-input edit-from-ref as before. The builder resolves the real path at build time; never a literal string placeholder.
   - `subject_role:` — (informational) `fixed-hero` | `free-subject`. `fixed-hero` blocks have NO free `{PHOTO_SUBJECT}` var (the `prompt_delta` recolors the ref); `free-subject` carries the per-post subject var. No special branching beyond honoring the `variables` list as written.
   - `render_register:` — (informational, IGNORE for generation) a builder-written echo of the style read; consumed by the post-render conference, not the generator.
   - `zone_position:` — `left:Xpx top:Ypx width:Wpx height:Hpx` (informational)
   - `brand_style_source:` — relative path inside `brand_context/` (e.g., `visual-identity/ai-image-style.md`) — the brand IDENTITY grade (palette / accent / grain) + the `default_*` style fallbacks
   - `image_style:` — THIS template's style, written by the builder from its ref: a nested `{ medium, lighting, subject_treatment }`. Any field may be absent → inherit the grade's `default_*` for that one field (the ambiguous-ref case). Absent block entirely → all three inherit the grade `default_*`. **NEVER** regress to using the whole grade as the style (the run-07 cause).
...
3. **Load the cues in TWO buckets — identity (brand) + style (template).** This is the run-07 fix: the grade is no longer poured whole into every slide.
   - **`identity_cues`** — from `{brand_context}/{brand_style_source}` (the grade): **palette / accent / grain** + the brand-wide cue prose. These are brand-wide; they give the feed its unity. Cache per run.
   - **`template_style_cues`** — from THIS block's **`image_style`**: **medium + lighting + subject_treatment**, the style of the template that is running. For any field `image_style` omits (or if the block has no `image_style` at all), inherit the grade's `default_medium` / `default_lighting` / `default_subject_treatment` for that one field only. **Never** fall back to the whole grade as the style — an absent `image_style` means "use the `default_*` fields", not "re-impose the entire grade" (that is exactly the run-07 regression: one brand-wide style stamped on a style-divergent template).
   - Render each bucket as a short cue string for the prompt (e.g. `identity_cues` → "in <palette>, <grain>"; `template_style_cues` → "flat vector illustration, natural light, full-bleed scene").
...
5. **Generate the image** at the size matching `output_aspect`. Apply the universal **Anti-AI-look** rules (negative prompts, forbidden keywords) from Step 6 regardless of route.
   - **edit-from-ref** — the ref carries the composition; `prompt_delta` carries only the subject. Pass the ref as `--input-image` (and the headshot as a SECOND `--input-image` when the block declares `identity_input:` — the `scene-restyle-with-real-face` ROTA, ref FIRST / identity SECOND):
     ```bash
     uv run .claude/skills/viz-image-gen/scripts/generate_image_gpt.py \
       --input-image "{template_dir}/{ref_input}" \
       [--input-image "{identity_input}"]   # only if the block declares identity_input (hero-face ROTA) \
       --prompt "{identity_cues}\n\n{template_style_cues}\n\n{prompt_delta_filled}" \
       --filename "{output_base}/{date}/{slug}/_slides/slide-{N}/_ai_bg/photo_main.png" \
     ...
     **Canonical output (Lane 1 fix):** the AI-bg MUST be written **into the post slide dir** (`{output_base}/{date}/{slug}/_slides/slide-{N}/_ai_bg/photo_main.png`) — NOT into `{output_dir}` (which at build time is the template dir) and NOT a project-root `slide-{N}-bg.png`. This is the file the slide actually ships; writing anywhere else gets discarded and the slide silently inherits the template's demo bg.
   - **legacy** — txt2img, no ref input:
     ...
     uv run .claude/skills/viz-image-gen/scripts/generate_image_gpt.py \
       ...
       --size {size_from_aspect} --quality high
     ```
     Same canonical output rule as edit-from-ref above: write into the post slide dir's `_ai_bg/photo_main.png`, never `{output_dir}` or a project-root temp.
     Provider selection: per `ai-image-style.md > provider` if present, else the legacy Step 5 / `sys-config.md` tie-breaker.

6. **Inject the generated path into render data** as the slot's value — wire **the exact file you just wrote**, never a separately-composed string:
   ```
   data[slot_path] = "{output_base}/{date}/{slug}/_slides/slide-{N}/_ai_bg/photo_main.png"
   ```
   **Lane 1 fix (raiz, não instância):** the slot value MUST be the resolved path the generator actually produced in `--filename` above — the post slide dir's `_ai_bg/photo_main.png`. Do **NOT** hand-compose a `templates/.../slide-{N}-bg.png` string from memory: a path decoupled from the real output never lands as `PHOTO_MAIN_PATH`, the generation is discarded, and the slide ships the template's demo bg. The path must point at a file that exists on disk; `render_template.py`'s `embed_paths_as_data_uris` inlines it and `emit_edit_slide`'s rewrite normalizes the `_ai_bg/` marker to slide-relative.

   **ONE canonical AI-bg per slot — never accumulate variants (bg-variant-divergence fix, raiz):** the AI-bg for a hero/photo slot is the SINGLE canonical `_ai_bg/photo_main.png`. If you re-generate (a second/third try, a quality bump, a different prompt), **overwrite the same `photo_main.png`** — do NOT write `photo_preview.png`, `photo_slide01.png`, or any other `photo_*` sibling into `_ai_bg/`. Extra variants accumulating in `_ai_bg/` are the bug: the bake resolves `{{PHOTO_MAIN_PATH}}` from the data you pin while the front/editor uses the slide HTML's static `src="_ai_bg/photo_main.png"` — pin a non-canonical variant and the shipped front image ≠ the baked preview. Always pin `PHOTO_MAIN_PATH` to `_ai_bg/photo_main.png` (the canonical name), never to an iteration variant. `emit_edit_slide` defensively normalizes a mis-pinned `_ai_bg` variant back to `photo_main.png` and skips copying non-canonical variants into the slide dir, but the generator MUST not create them in the first place. (Exception: a HYBRID_REAL slot may keep a `*_setup` companion — clearly NOT the PHOTO_MAIN — alongside `photo_main.png`.)
...
### `--emit-edit-slide` — REQUIRED on every templated bake (AIOS-139 Addendum 8 #1)

Always pass **`--emit-edit-slide`**. For Case A (HTML) and Case C (hybrid) slides it writes a self-contained editing dir next to the PNG — `{output_base}/{date}/{slug}/_slides/{slide-id}/` (`template.html` + `instructions.md` + `metadata.json` carrying the slide's REAL `--data`) plus a once-copied `_slides/_shared/`. For Case B (full-AI `.prompt.md`, no HTML) it is a silent no-op.

This is the contract Content Studio (Phase 7.5) reads: without it, a real run folder holds only `slide-*.png`, so templated slides fall back to a flat read-only image (no text/layout controls, `/apply` "not rebakable"). With it, the editor renders each templated slide as live HTML with introspected per-zone controls and rebakes the **real** copy.

> **Clean-output policy — documented exception.** `_slides/` is the ONE editing-state directory allowed to persist in the run folder (alongside `tweaks.json`/`comments.json`). It makes the run self-contained (survives `brand_context` moving) and is what powers re-editing. Do not delete it in any cleanup step.
...
**Path C — `FULL_AI`:** skip the template overlay entirely. The AI generates the complete image with text baked in. Output goes directly to the final filename. If `ai_style == "dense-infographic"`, use Path D below instead (multi-input edit-from-ref + verbatim prompt + verification gate).
...
**Data mapping for every slide — the designer's slot maps are the source of truth.**

Build each slide's `--data` JSON in THIS ORDER. Later steps override earlier ones — the designer's per-slot values always win:

**1. Canonical defaults** (back-compat for the known photo-overlay / editorial-news / cta families; harmless extras for templates that don't declare them — `:empty{display:none}` hides unused slots):
...
**2. Merge `slide.slot_text` wholesale — MANDATORY, slot-vocabulary-agnostic.** ssc-designer emits a `slot_text{}` map with one entry per editable TEXT slot the chosen `template_id` declares, keyed by the template's OWN slot names (`HEADLINE`, `KICKER`, `ANCHOR_WORD`, `CTA_TEXT`, `SUBTEXT`, `CAPTION`, or any slot a builder-authored template defines). Copy EVERY entry into `--data` as-is — do NOT hand-map or filter to a known token list. This is what lets a pool template the generator has never seen render correctly: the designer already named its slots. `slot_text` values win over the canonical defaults above (a template that uses `HEADLINE` instead of `TITLE` gets its text from `slot_text.HEADLINE`). **Without this merge, every slot outside the canonical vocabulary renders empty — the AIOS-190 flow-2 render gap.**

**3. Merge `slide.template_slot_values` wholesale.** For templates with `[ai-image-zone:N]` blocks, the designer emits `template_slot_values{}` (e.g. `CONTENT`, `AI_SUBJECT`, `ANNOTATION_LABEL`). Copy every entry into `--data` too — they fill the composition prompt's variable slots (Case C hybrid render).

The per-family `--data '{...}'` examples below are illustrative DEFAULTS for the legacy families. Whenever the slide carries `slot_text` / `template_slot_values` (every pool-mode slide from ssc-designer does), those maps are the real payload — merge them in per steps 2–3, don't replace them with the hardcoded example tokens. HTML pass-through: `slot_text` values may contain `<mark>`/`<br>`/`<em>` — pass unescaped (the renderer's triple-brace slots + brand `mark` CSS handle them).
...
**`FULL_AI` (social-design only, FRONT only):** skip the template overlay entirely. The AI generates the complete slide with text baked in. Build the prompt per Step 5.5a (FULL_AI ending). Output goes directly to `slide-1.png`. If `ai_style == "dense-infographic"`, use Path D below instead (multi-input edit-from-ref + verbatim prompt + verification gate):
...
**Path D — `FULL_AI` dense-infographic (cover-framework-diagram).** Triggered when `ai_style == "dense-infographic"` AND `render_mode == FULL_AI` (single image OR carousel FRONT). No template overlay, no `_tmp-*.png` — the AI output IS the deliverable. This uses GPT edit-from-ref with the canonical reference as INPUT 1 and the verbatim-text prompt.

Step D.1 — Resolve inputs (fidelity ladder: real asset > web > AI):
- INPUT 1 (required): the canonical reference at `brand_context/visual_refs/framework-diagram-ref.png`. If it is missing, log `dense-infographic: no canonical reference — degrading to text-only generation` and proceed with no `--input-image` (AI composes from prompt alone).
- INPUT 2 (optional): a real headshot. Resolve from `brand_context/visual-identity/headshots/` (first match), else `brand_context/visual_refs/` by filename containing `headshot|author|founder|face`. If none, OMIT INPUT 2 (no-asset fallback, ASSET-03).
- INPUT 3 (optional): a real screenshot/frame. Use `slide.frame_path` if set, else `brand_context/visual_refs/` by filename containing `screenshot|terminal|ui|mock`. If none, OMIT INPUT 3.
- Logo tiles (LOGO-01): for each of the N logo names, resolve in order — (1) bundled local icon under `.claude/skills/viz-image-gen/references/icons/commons/` (matching `ai/`, `social/`, `dev/` paths as in Step 5.7 Tier 4); (2) if absent, fetch via `uv run .claude/skills/tool-image-search/scripts/search.py --query "{logo_name} logo" --intent brand --count 3 --output-dir "{output_base}/{date}/{slug}/sourced/"` and pick the first result; (3) if both fail, let the AI draw the logo tile from the prompt name. Logos may be passed as additional `--input-image` when a clean SVG/PNG resolves, OR named in the prompt for AI to draw — prefer the resolved asset for brand exactness.

Step D.2 — Build the verbatim-text prompt. Use this exact structure (slot values come from the slide plan's dense-infographic slot map: KICKER, TITLE_ACCENT, TITLE_MUTED, NODE_LABEL, N cards each {LABEL + M bullets}, N logo names, N captions, BYLINE). N ∈ 2–4, M ∈ 2–4.

```
Build this framework-diagram infographic with rich production quality, re-skinned to
'{brand_name}' brand: {background_hex} background, {accent_hex} accent, clean modern
sans-serif, flat editorial-tech design, soft shadows and depth.

{asset_preamble}

Render ALL other text EXACTLY and VERBATIM, do not paraphrase, do not harmonize bullets
to match each other, do not change a single word:
- Top {accent} pill: {KICKER}
- Headline two lines: {accent} '{TITLE_ACCENT}' then grey '{TITLE_MUTED}'
- Center grey label: {NODE_LABEL}
- {N} cards ({per-card top-accent colors}), {M} bullets each verbatim:
  Card 1 '{LABEL_1}': {bullet_1_1} / {bullet_1_2} / ... / {bullet_1_M}
  ... (one line per card, up to card N) ...
- {N} white logo tiles for: {LOGO_1}, {LOGO_2}{, {LOGO_3}}{, {LOGO_4}}
- {N} dashboard thumbnails captioned verbatim: {CAPTION_1} / {CAPTION_2}{ / ...}
- Bottom-left: {byline_clause}
No other text. Spelling exact.{n2_suffix}
```

Where:
- `{asset_preamble}` when INPUT 2 AND INPUT 3 resolve = the three-input block:
  `THREE input images are provided:` + the INPUT 1 / INPUT 2 / INPUT 3 lines from the A3 recipe (INPUT 1 = COMPOSITION/LAYOUT reference; INPUT 2 = REAL author photo -> circular bottom-left avatar, keep the real face, do not redraw or stylize; INPUT 3 = REAL screenshot -> inside the product-mock panel, keep readable, lightly color-grade to theme, do not invent fake terminal text).
  When only INPUT 1 resolves (no real assets) = `ONE input image is provided:` + only the INPUT 1 line. Drop INPUT 2/3 references everywhere; AI invents the avatar/mock (ASSET-03).
  When INPUT 1 missing too = omit the preamble entirely.
- `{byline_clause}` = `the real avatar (INPUT 2) + grey text: {BYLINE}` when INPUT 2 resolved, else `grey byline text: {BYLINE}`.
- `{n2_suffix}` = ` Balanced two-column composition.` when N == 2 (T3 proof), else empty. (TMPL-01 balance.)
- Theme-agnostic (TMPL-02): the slot values are the ONLY content source — never re-inject the original source theme; swapping slot copy produces the same composition with new content.

Step D.3 — Invoke (canonical ref FIRST, headshot SECOND, screenshot THIRD — Pitfall 4):

```bash
uv run ".claude/skills/viz-image-gen/scripts/generate_image_gpt.py" \
  --prompt "{verbatim prompt from D.2}" \
  --filename "{output_base}/{date}/{slug}/{slide-1 or image}.png" \
  --size 1024x1536 --quality high \
  --input-image "{INPUT 1 path}" \
  [--input-image "{INPUT 2 path}"]   # only if headshot resolved \
  [--input-image "{INPUT 3 path}"]   # only if screenshot resolved
```

Then run the text-verification gate (Step 8.5) BEFORE the slide is considered done.
...
## Step 8.5: Text-verification gate (FULL_AI dense-infographic only)

**Only run when the slide was rendered via Path D (`ai_style == "dense-infographic"`, `render_mode == FULL_AI`).** This gate is AUTOMATIC and BLOCKING — no slide with baked text is presented before it passes.

G.1 — Build the intended-values map: every text string injected into the Path D prompt, keyed by role: `kicker`, `headline_accent`, `headline_muted`, `node_label`, `card_{i}_label`, `card_{i}_bullet_{j}` (for i in 1..N, j in 1..M), `logo_{i}`, `caption_{i}`, `byline`.

G.2 — Read the rendered PNG with agent vision (the agent runs `model: sonnet` and has the Read tool). Prompt yourself:
  `Read the file: {output_path}. List every visible text string exactly as rendered. Return a JSON object keyed by visual role (kicker, headline_accent, headline_muted, node_label, card_1_label, card_1_bullet_1, ..., logo_1, caption_1, byline).`

G.3 — Diff. For each intended value, NORMALIZE both the intended string and the extracted string the same way: lowercase, strip all punctuation, collapse whitespace. Compare normalized forms.
  - A MISMATCH is a genuine word substitution after normalization (e.g. intended `Hermes storage` vs rendered `Hermes recall` -> mismatch on the second token). A pure case/punctuation difference (e.g. `storage` vs `Storage`) is NOT a mismatch (Pitfall 3 — prevents false-positive re-rolls).
  - Record every mismatch as `{role, intended, rendered}`.

G.4 — On any mismatch, re-roll/fix with a bounded budget:
  - Attempt 1: full re-roll — re-run Path D Step D.3 with the same prompt (new generation). Re-verify from G.2.
  - Attempt 2: full re-roll again if still failing.
  - Attempt 3: targeted fix — re-run Path D Step D.3 with an appended correction line: `IMPORTANT CORRECTION: the text in the {role zone} must read EXACTLY '{intended}', not '{rendered}'. Do not change any other text.` Re-verify.
  - After 2 re-rolls + 1 targeted fix still failing: stop, log `dense-infographic verification FAILED after 3 attempts`, and flag the slide `needs-user-decision` in `pipeline-log.md`. Do NOT silently present an unverified slide.

G.5 — Log the gate result per slide in `pipeline-log.md`: `text-verification PASS` (with attempt count) or the failure flag. Only a PASS slide is returned as a final path.
