...
**Pool mode is universal (Stage 2).** Every slide carries `template_pool` + `template_id`. Legacy `template_family + layout` is retired — do NOT emit those fields. Each pool's `manifest.json` lists the templates and their slot/role metadata; the designer's job is to walk that manifest and pick the entry that fits each slide's role + content shape. The renderer (`render_template.py --template-pool <pool> --template-id <id>`) resolves the actual HTML file from the manifest entry's `file` field, which may point inside the pool's own folder (`heroes/hero-typographic.html`) or cross-import another template's folder via `../photo-overlay/front.html`.
...
When the chosen `template_id` has one or more `[ai-image-zone:N]` blocks declared in its `template.html` (see `.claude/skills/00-social-content/references/template-authoring/shared/template-conventions.md > AI image zones`), the designer's responsibility for that slide is to **fill the variable slots only — never compose full AI prompts**.
...
**Exception — `dense-infographic` archetype (cover-framework-diagram).** When `ai_style == "dense-infographic"`, the FULL_AI eligibility gate is RELAXED: the `Headline ≤ 8 words AND ≤ 60 chars` cap and the `body == ""` requirement do NOT apply — this archetype legitimately bakes ~20 dense labels (kicker, two-line headline, node label, N cards × M bullets, N logo tiles, N captions, byline). The slide is FULL_AI unconditionally. All OTHER eligibility checks still apply: headline sanitization (no leading hyphens, curly quotes, ellipses, emoji, brand placeholders) and the `brand_name`-not-a-literal-string rule. The archetype/number gate (`archetype ∈ {03, 05, 09, 17, 20, 23}`) does NOT apply because `dense-infographic` is its own `ai_style`, not a social-design archetype.

Density: `dense-infographic` supports N ∈ 2–4 cards and M ∈ 2–4 bullets per card and must stay balanced (N=2 → balanced two-column composition). The cap removal applies to the full N×M label set, never to the unrelated short-headline `social-design` cover archetypes — those keep the ≤8-word/≤60-char cap.

Audit-log line on application: `dense-infographic FULL_AI exemption applied — headline cap and body-empty checks bypassed`.
