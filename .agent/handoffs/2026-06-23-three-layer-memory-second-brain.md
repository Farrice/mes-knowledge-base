...
- `_active/harness/memory-bakeoff/04-deliverables/bake-off-protocol.md` (NEW) — locked decision rule; incumbent episodic-memory wins; hard DQ for ungated paid calls. Fast-path (adopt incumbent directly) recommended.
...
1. **Commit** the uncommitted work (offered at end of /end-session): `execution/memory_facade.py`, `execution/notion_api.py`, `execution/episodic_ingest.py`, `CLAUDE.md`, `.env`, `_active/harness/memory-bakeoff/`, `_active/knowledge/notion-intellectual-library/04-deliverables/DEPLOY-RUNBOOK.md`.
...
- `_active/knowledge/notion-intellectual-library/04-deliverables/DEPLOY-RUNBOOK.md`
