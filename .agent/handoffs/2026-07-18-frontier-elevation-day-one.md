...
- Batch 6 (16: forge-os→jack-roberts) CLOSED across a SESSION-LIMIT INTERRUPT
  (2026-07-17 ~11:50pm): 5 merged pre-interrupt; 11 workers killed mid-task. RECOVERY
  PATTERN (now proven): strict-gate-test every surviving delivery (7/11 payloads were
  complete — merged as salvage, no worker report, tagged priority-verify), revert
  partial copies, redispatch the rest (4) with predecessor findings in the prompt.
  Salvage verify 3/3 CLEAN. One redo worker found its dead predecessor's draft had
  FABRICATED-looking citations and discarded them per envelope — gate salvages, never
  blind-merge. 96/279 drained after b6.
- Batches 7-10 CLOSED (160/279 total): b7 verify caught the OVER-BROAD jim-osh
  exposure (apostrophe broke grep — real source existed; correction merged, queue
  entry retracted); b8 caught + redid welsh's false 'unrecoverable' claim (54 tarball
  members existed); b9 verify confirmed workers' fabrication discoveries (fake Cell
  Metabolism cite, $1M overclaim); b10: strict fleet_merge caught its first live miss
  (regex-mismatched recognition phrasing, conductor-aligned). WORKER PROMPTS now carry
  SOURCE-SEARCH DISCIPLINE (name fragments w/o punctuation, python tarfile per-member
  scans, record sizes) — false-absence rate dropped to zero after it landed.
- Batches 11-13 CLOSED (208/279): b10/11 verify caught lakajev's silent deletion +
  false 'flagged not deleted' disclosure (restored + ledger corrected); b12 strict gate
  caught a 0.21 entity ratio (fixed by quoting the skill's own framework term); b13
  caught oscar-hoglund's soft-wrap problem. WORKER PROMPT RULES now include: anchors
  ON the list-item line, literal "recognize this as", entity regex needs
...
4. Source re-acquisition queue (video 412qINvYIKk, Georgi masterclass, Diandra
   primary, Cole Two-Rules) — jim-oshaughnessy RETRACTED from this queue 2026-07-18: the 'no source'
   exposure was a grep false-negative (apostrophe in O'Shaughnessy); real source found,
   15/17 patterns VERIFIED. Lesson: search surname fragments without apostrophes.
