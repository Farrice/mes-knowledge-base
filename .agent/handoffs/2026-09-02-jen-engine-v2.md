...
status: active
resume_hint: Jen Engine v2: read _active/clients/jen-listings/06-system/ENGINE-V2.md, fold research if landed, build first two weeks in photo look from Drive folder 01
branch: worktree-jen-carousel-reel-concepts
pin: false
---
...
## Remaining Priority
Build the first two weeks of finished posts in the photo look from her listing photos once folder 01 has them: three posts a week (attract with a price-signal hook, position, convert), captions, saved replies, delivered to Drive folder 04 by week. Show Jen the reel, not the spec.

## Do-NOT-Rebuild
Valley Native line-drawing system (shelved, not deleted). The 16-board deck and narrated video (done). The five FTHB topic cards (off-thesis; mine them for lines only). Never a weekly ask on Jen; never name the machine to her.
