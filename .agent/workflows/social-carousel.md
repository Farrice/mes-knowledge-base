...
## Proven run shape (blind bar 01, 2026-09-03 — Farrice: "11 out of 10... this needs to be the floor")
The order that produced it, in one breath: BRAND LOCK → research with receipts into their cache → REAL captures of the primary surfaces (Ad Library filtered to the brand's page id, product pages, court-record pages), cropped to the slot's shape → claims ledger (anything not on it does not ship) → our pens write caption + 7-slide script with template ids and every slot filled, copy FINAL → claim_audit --strict on the take file + prose_classifier on the caption alone → ssc-designer with the script as inspiration_pool and "copy is FINAL, photos are these USER_ASSET paths" → render with the slot rules, --data as a file → contact-sheet read and one correction pass before he sees anything → blind judging surface (takes as 1/2/3, his approved frames as the bar, key in a sidecar) → his verdicts into voice_ratchet.py verbatim. Full card: `docs/solutions/2026-09-03-teardown-carousel-copy-lock-evidence-crops.md`. Default the Farrice-brand teardown to the NAMED brand with the fight picked; the anonymized composite is offered only when client privacy forces it (his verdict: "safe... in the middle content gets killed and buried").

Two steps his critique added (binding from run 02 on):
- **Step 2.5 — Operator verification hub.** Before hand-off, write `VERIFICATION.md` in the run folder: every claim used, its tag, its source URL, and the evidence crop it rides on (thumbnail path). Operator-only; never on the slides. He reads it in one pass to sign off on accuracy. "We lose all credibility if there's no verification."
- **Step 4.5 — Composition pass against the frames.** Read the contact sheet next to `brand_context/visual_refs/editorial/`. Check: proportion and crowding per slide, one hierarchy per slide, and every evidence crop shows the part that matters (pre-crop to the slot's aspect ratio; object-fit cover otherwise cuts it). Fix at the crop/template level, never by softening copy. Re-render only the affected slides and log the correction.
