...
2. Integrator per `pens.integrator` writes ONE take: the caption (narrative → facts → door → attribution for Jen; VOICE-CARD register for Farrice) and a slide script — for each slide `SLIDE n | role | headline (≤8 words) | body (≤40 words) | image concept`. Cover hook = the chosen hook, verbatim.
...
- Every `render_template.py` / `content_studio.py` call carries `--brand-context <brand_context>`.
