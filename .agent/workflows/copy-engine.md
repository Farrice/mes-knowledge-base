...
**Buyer-decision intelligence sniff (cold SHADOW):** using only the dossier, `warm_core`, offer facts, and proof state already present, ask whether one eligible **Belief, Focus, Recognition, Congruence, or Affect** decision would materially change a copy block, its sequence, proof placement, or CTA. If yes, cold-load `semantic_libraries/antigravity/primitives/buyer-psychology-decision-intelligence-overlay.md`, select one primary decision, and return the smallest source-bounded change to this workflow before block sourcing. `/copy-engine` remains the owner. Fit and Choice return to the offer or campaign owner; Priority returns to the content, writing, or client-delivery owner. Do not infer buyer psychology, add a gate or question, weaken G0/G2/G5, or let the overlay override market evidence, claim status, proof, voice, terms, or high-stakes verification. The five `CANDIDATE` cards are development-only.
