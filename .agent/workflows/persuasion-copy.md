---
description: Persuasion-Engineered Copy Engine
---

# Persuasion-Engineered Copy Engine

Run the McRaney Persuasion-Engineered Copy Engine.

## Steps

1. Load: `skills/david-mcraney-belief-change/genius.md` + `skills/luke-iha-proof-ladder/SKILL.md` + `skills/luke-iha-copy-blocks/SKILL.md`
2. Load workflow: `skills/david-mcraney-belief-change/workflows/persuasion-engineered-copy-engine.md`
3. Execute with inputs: Offer/Product, Audience, Copy Type, Known Objections (optional), Existing Research (optional)
4. Produce: Belief Diagnosis, Accommodation Design, Copy Architecture, Proof Prescription, Finished Copy, Accommodation Audit
5. Finalize via chain_runner.py
