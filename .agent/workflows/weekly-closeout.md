...
- **Verifier fleet**: `python3 execution/verify_fleet.py status` — the Sunday train (launchd `com.antigravity.verify-fleet`, Sun 05:30) runs every `verify_*.py`; failing verifiers listed here are broken CONTRACTS, not noise. Triage each: real drift → fix the system; stale expectation → fix or propose-archive the verifier via pending-review.
