...
| L2 | claude | Parity spec | complete | the exact job he runs in the paid tool (his tab, or his URLs + ask), written as a fixture: inputs, the ask, what a good answer contains | .agent/missions/poppy-canvas-verdict/parity-fixture.md (Jen board observed + Run 2), jen-poppy-thread-raw.md, jen-poppy-thread-full.txt, jen-poppy-thread-2-instagram-agent-structure.md |  | packet 1 answered A but macOS Screen Recording is not granted to the Claude app and the Chrome extension is not connected — grant one, or paste the Jen URLs + ask |  |
| L3 | claude | Parity run | complete | the fixture through the local version; time, tokens, cost, and the answer beside the paid tool's | Run 2 on his real fixture: board jen-sfv, 5 Poppy videos + voice note, 77K tok, 355 s, est $5.13 on plan; reply jen-canvas-reply-run1.md; gap = no web tool in seat; autocompact thrash bug fixed in canvas_board._seat_claude |  |  |  |
