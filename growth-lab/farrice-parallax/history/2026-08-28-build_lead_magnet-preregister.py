...
def prune_pack(pack, niche_label):
...
def derive_whitespace(pack):
    ...
    if len(formats) >= 3:
        ...
        if cands:
            ...
            desire = best.get("desire_template")
            pair = f"“{best['hook_format']}”" + (f" carrying “{desire}”" if desire else "")
            return {
                "kind": "format",
                "headline": "An open format lane",
                "body": (f"The {pair} open averages an outlier score of {best['avg_score']:.1f} "
                         f"but shows up only {best['count']} time(s) in this sample. "
                         "The lane is working and nobody is crowding it yet."),
    ...
    if len(topics) >= 2:
        ...
        if cands:
            ...
            return {
                ...
                "body": (f"“{best['topic']}” earns an outlier score of {best['score_sum']:.1f} "
                         f"from just {best['video_count']} video(s), while the busiest topic here has "
                         f"{max_count}. The pull is proven; the shelf is nearly empty."),
...
def render_enrichment_nuggets(nuggets):
    ...
    for n in nuggets:
        if n["kind"] == "topic_demand":
            head = f"Demand is moving on &ldquo;{esc(n['topic'])}&rdquo;"
            if n.get("trend_direction") == "rising":
                head = f"Rising demand: &ldquo;{esc(n['topic'])}&rdquo;"
        else:
...
def render_topic_rows(topics):
    ...
    for i, t in enumerate(topics, 1):
        ...
        meta = f"{count} video{'s' if count != 1 else ''} &middot; score {score:.1f}" if isinstance(count, int) else f"score {score:.1f}"
        rows.append(
            '        <li class="topic-row">\n'
            f'          <span class="tr-rank">{i:02d}</span>\n'
            f'          <span class="tr-name">{esc(t.get("topic"))}</span>\n'
...
def render_teaching_panel(mini, mode):
    ...
    if mode == "full":
        ...
        if mini["topics"]:
            t0 = mini["topics"][0]
            count_bit = f" across {t0['video_count']} video(s)" if isinstance(t0.get("video_count"), int) else ""
            beat3 = (f"Pull concentrates. “{esc(t0['topic'])}” holds the most outlier pull in this "
                     f"sample{count_bit}. A scoreboard like this is demand talking &mdash; before you spend a single filming day.")
        else:
            beat3 = ("Pull concentrates &mdash; a few topics usually hold most of the outlier energy. "
                     "This sample was too thin to rank them; the next refresh will.")
    ...
    if mode == "full":
        ...
        if mini["topics"]:
            blocks.append(_block(
                ...
                "Topics ranked by total outlier score across the sample &mdash; demand talking, before you film anything.",
...
def bake(template_path, pack_path, niche_label, cta_url, out_path):
    template = Path(template_path).read_text(encoding="utf-8")

    pack, reason = load_pack(pack_path)
    if pack is not None:
        mode = "full"
        mini = prune_pack(pack, niche_label)
        niche_slug = pack.get("niche_slug")
    else:
        mode = "interview"
...
def main():
    ...
    mode, reason, mini, dest = bake(args.template, args.pack, args.niche_label, args.cta_url, args.out)
    ...
    print(f"[build_lead_magnet] wrote {dest} — mode={mode} videos={len(mini.get('videos', []))} "
          ...
          f"{enrich_bit}pack_reason=\"{reason}\" cta_host={cta_host} $0")
    return 0
