...
**Antigravity equivalent:** Root `CLAUDE.md` ✓. Client folders exist (`projects/javier-human-values/`, `projects/jen-santulan/`, `_active/farrice-brand/`, etc.) but **`find` returned ZERO per-client CLAUDE.md files** — no inheritance is actually wired up. Skills live at root `skills/`, available to all projects but not per-client customizable.
