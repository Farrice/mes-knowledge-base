# Resonance — Brand Library Entry

*Entry for `knowledge/design-libraries/brands/resonance/`. Format-matches the existing 59 entries in the brand library (Apple, Stripe, Linear, Aimé Leon Dore, etc.). Used by the design-md tooling, by future Creative Director sessions, and by any agent that needs Resonance context loaded as a Tier 1 reference.*
...
**One-line positioning:** *Heart encounters, not head encounters — a daytime, sober dance party in Chicago for people who want to meet a partner.* (Manifesto v2 epigraph.)

**Paragraph positioning:** Resonance is a curated daytime dance room for 30-to-40-year-old single Chicagoans who want a committed partner and are tired of formats that make that harder, not easier. The mechanic is body-first: the music does the emotional labor so the people don't have to. The metric is couples, not followers. The brand's enemy is not a competitor — it is the culture of head-first, transactional, alcohol-mediated meeting that the ICP is already exhausted by.

**The wedge:** Resonance is the only daytime, sober, curated-music, curated-crowd dance event in Chicago that explicitly measures itself by couples formed (25% target per event), not by attendance or followers. Every other daytime-dance brand in the category (Daybreaker, Morning Gloryville-era sober raves) is wellness-coded; every other singles-event brand is bar-coded. Resonance refuses both costumes.
...
# Hand-script — Caveat (rare punctuation only — Andrea's signature, couples' quotes)
...
When an AI session loads Resonance via `python3 execution/design_md_brand_lookup.py use resonance`, the system inherits:
...
For Creative Director chains, this entry is the Tier 1 brand context. Pair it with the Brand Bible §8 (strategic intent) and the Manifesto v2 (voice and phrases) to do any Resonance creative work.

## Quality Bar (the discipline this entry holds)

Every Resonance design decision must clear:
...
3. **The voice test.** *Could Andrea show this to a friend who lives the room and have them recognize it?* If no, kill it.
