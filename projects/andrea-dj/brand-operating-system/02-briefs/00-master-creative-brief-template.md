...
1. **Profile**: Nora (#1 LOCKED, arts worker) / Imani (#2 PROPOSED, from Simone) / Marcus (#3 PROPOSED, from Darius) / All-three / Umbrella ("the out-loud-asking 30-40 hell-yes seeker"). See `00-foundation/02-icp-master.md` for full profiles.
...
TASK: [the specific request — e.g., "draft a 200-word IG feed post announcing Event #1 to Nora in contemplation state. Lead with out-loud-asking opener. End with hell-yes filter."]
...
2. **Profile fit**: Would the targeted profile (Nora/Imani/Marcus) recognize themselves? Or is this generic?
3. **Voice match**: Could Andrea say this to a friend over coffee? If no, rewrite.
...
- Which Andrea source docs (anchor / manifesto v2) the spine pulls from
- Any examples from prior assets that succeeded (post-Event #1, this becomes a richer reference layer)

This isn't bureaucracy — it's how amendments cascade. When Andrea names an amendment, the cited docs surface and the brief updates.
...
- Not the only structure that works — but it's the one Resonance uses to keep coherence across 9+ asset types
