...
| `projects/Ken's Fasting Digital Product/swarm_research/metadata.json` | Proof of false-PASS era: all 5 agents `grounding_queries: 0`, gemini-2.5-flash, 20260331. | Treat all downstream Ken outputs as ungrounded; re-run. |
| `projects/Ken's Fasting Digital Product/swarm_research/synthesis.md` | Built on zero-grounding run; itself admits "lack of credible scientific evidence," no URLs. | Re-ground; do not ship health claims. |
| `projects/Kens_Fasting_Digital_Product/04-deliverables/landing_page_copy.md` | **Shipped sales copy**, unsourced health claims + results guarantee that its own synthesis demanded removing. **Medical-claim liability.** | Pull guarantee/"exact formula" language now; re-ground before relaunch. |
| `projects/unbottlenecked_blueprint/04-deliverables/06_icp_intelligence_report.md` | "5-Agent VoC Swarm," ZERO URLs, ZERO verbatims; modeled language drives $15k + $99/mo offer decisions. | Re-ground via `/avatar-machine`; re-validate offer assumptions. |
...
| `projects/teardown-kajabi/state.yaml` | anchor-001 = "GROUND dossier (DEGRADED)"; 4 copy blocks ref a failed foundation. | Re-anchor to a re-grounded dossier before producing copy. |
...
| `projects/teardown-kajabi/state.yaml` *(also HIGH above for the dossier)* | — |
...
| `projects/_ck2free/state.yaml` | anchor-001 = "FREE-DIRECTIVE" (un-grounded); referenced dossier **does not exist on disk** (orphaned anchor). |
| `projects/sfv-first-time-homebuyers/state.yaml` | DEGRADED grounding never linked back — orphaned from its (failed) provenance; empty deliverables/research dirs. |
| `projects/teardown-skool/state.yaml` | Initialized 2026-05-31, `anchors: []`, no Phase 0 GROUND recorded. |
