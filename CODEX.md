...
- Completed closeout steering uses `execution/contextual_next_prompts.py --format compact` to produce exactly three ranked, session-specific prompts with a concrete outcome, a short why-now line, and a copy-ready prompt. Approval-blocked or partial closeouts show only the exact blocker, unarchived state, recoverable artifact path, and one approval sentence; they do not render a continuation menu.
