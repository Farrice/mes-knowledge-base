...
|---|---|
| **Fable / Mythos-class (5.1 charter 2026-09-11)** | Full doctrine as written. Novel-pattern authoring allowed freely (new workflow scripts, new gates) — but bank anything reusable into directives/solution cards before session end. **Manager, not pen:** Fable turns are plan · brief · verdict · integration; mechanical and parallel tracks go to seats via `job_board.py dispatch` (Sonnet in-session) or `job_board.py run` (sonnet / opus / codex headless, his subscriptions). Every job leaves a board a weaker conductor can execute unsteered. |
| **Opus-class** | Full doctrine as written — this is the EXPECTED steady-state conductor, not a degraded mode. **Board-first (2026-09-11):** on a job with a Fable-authored board, Opus executes `/job resume <slug>` by the briefs and never re-plans; judgment → DECISION PACKETS. Prefer proven machinery (existing wave scripts, gates, queue builders) over authoring novel orchestration mid-mission; when a genuinely new pattern is needed, wargame it first (`/wargame-run`) or run a smaller proof. Fast mode is fine for conducting; it's still Opus. |
