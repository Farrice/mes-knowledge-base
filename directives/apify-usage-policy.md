...
| `linkedin-post-reactions` | `apimaestro/linkedin-post-reactions` | Who reacted to a post (engager signal) | $0.25 | pay_per_event | Added 2026-08-06 (Cody Schneider extraction). Schema verified via `builds/default/openapi.json`: `post_urls` (array, 1–1000), `page_number`, `reaction_type` (ALL default), `limit` (1–100/post/page). Returns reactor `{urn, name, headline, profile_url}`. Primary consumer: `execution/signal_scout.py` (LISTENING-ONLY). |
| `linkedin-post-comments` | `apimaestro/linkedin-post-comments-replies-engagements-scraper-no-cookies` | Who commented on a post + comment text (ICP verbatim) | $0.25 | pay_per_event | Added 2026-08-06. Schema verified: `postIds` (array, required, 1–500), `page_number`, `sortOrder` ("most recent" default), `limit` (1–100/post/page). Rich author + stats + reply payload. **Observed 2026-08-06: full scout run (4 posts × 30 reactions + 31 comments, incl. 2 posts-fetches) = $0.50 total.** Primary consumer: `execution/signal_scout.py`. |
| `twitter` | `kaitoeasyapi/twitter-x-data-tweet-scraper-pay-per-result-cheapest` | X/Twitter search + timeline | $0.25 | per_result | 4.58★. **Observed cost 2026-08-05: $0.25/1k tweets** (15 tweets → $0.0037). Registry uses 0.00025/result. ⚠ **Data-quality flag (2026-08-05)**: on empty results the vendor pads the response with identical "mock data" notice items (their minimum-charge policy) — consumers MUST filter items whose text mentions "KaitoEasyAPI"/"mock data" before synthesis; if this recurs, swap to `apidojo/twitter-scraper-lite` ($0.15/1k). |
...
- **LinkedIn post reactions/comments**: signal-based listening — "engagement is a hand-raise" (Cody Schneider doctrine). Feed `execution/signal_scout.py` → engager rosters + resonance reports + ICP-verbatim language for the Angle Brief. **Never for automated outreach — sends stay human (Farrice, 2026-08-06).**
