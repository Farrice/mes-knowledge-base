...
## Law 0 — One driver per tree (prevention beats merging)

- Any build/forge/fleet/multi-file session **claims the lock first**:
  `python3 execution/session_lock.py claim "<mission>"` — and heartbeats between waves.
- `session_lock.py check` before wave-building. A fresh foreign lock = do not build; wait,
  coordinate with Farrice, or move to a `git worktree`.
