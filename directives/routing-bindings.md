...
| Social/audience/trend listening ("what's happening in [niche]", "audience sentiment", "creator analysis", "#hashtag trends", "social listening", "social listening", "social sentiment") | **Apify-first pipeline** — `/social-listen` (on-demand) or `/social-pulse` (recurring weekly). Actor selection automatic per query type (transcript actors for depth, hashtag actors for trends, profile actors for creators). Synthesis via Perplexity. **This is the Apify→synthesis pipeline per apify-usage-policy.md** | Generic web research (Apify beats generic search for JS-rendered, community sentiment, raw creator data). Research synthesis without raw scraping (missing the extraction layer). |
