...
| Task-shaped ask, any raw dump ("autopilot", "no gates", "just execute", or no prefix at all) | `/autopilot` — THE task door (2026-09-11): mirror · shape check · compile · route · 3 code gates only (G1 intent <=2, G2 cost >$5, G3 prose FLAGGED at Expert Standard >=7) · run · receipt. `--preflight` = the /go MISSION CARD sign-off | Job-shaped → `/job`; deliberation → `/convene`; `/supercomputer`, `/jcc-deploy` are conductors it routes TO |
