...
| Generic research · "deep research on X" · strategic intelligence | `/deep-research-os --free-first`: Codex native web and opened current sources first; bounded basic-depth Tavily Search/Extract and public RSS for identified gaps; local context and skills only after the live-evidence gate. Every result carries evidence, quality, quota, and value receipts. | Training-memory answers; unbounded paid pulls that skip the cheapest-first ladder (`directives/live-data-grounding-protocol.md`: free floor → one bounded cheap Apify pull per `apify-usage-policy.md` → paid depth only with explicit authorization) |
| Social/audience/trend listening ("what's happening in [niche]", "audience sentiment", "creator analysis", "#hashtag trends", "social listening", "social sentiment") | On-demand `/deep-research-os --free-first`: public native-web evidence, opened pages, bounded basic Tavily Search/Extract, and public RSS/Atom where useful. Inaccessible platform-private data remains an explicit evidence gap. | Skipping the cheapest-first ladder (free floor → ONE bounded cheap Apify pull per `apify-usage-policy.md` → search synthesis); `/social-pulse` schedules; unbounded paid synthesis; invented private-platform coverage. Precedence rule (2026-08-21): the COST LADDER governs escalation — tool bans never do; `apify-usage-policy.md` + `live-data-grounding-protocol.md` are authoritative for when a paid pull is allowed |
