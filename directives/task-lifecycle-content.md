...
2.7. **Blind Bar pass (`directives/blind-bar-protocol.md`, Farrice-approved 2026-08-05).** If the brief names a Bar artifact (golden ref / rubric-anchor example / register exemplar): set the draft blind side-by-side with it and answer the falsifiable question — which clears the bar? Draft loses → name the single biggest gap, repair it, re-check. Cap: 2 rounds, then deliver with the gap named. In-context only (~5–15k tokens), deterministic checks (step 2) run first, no subagents. No named Bar → `BlindBar: N/A` and move on — never improvise a reference. Nudge + auto-repair, never a gate.
