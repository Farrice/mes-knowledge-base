...
type FlywheelState = 'IDLE' | 'AWAITING_STEP1_CONFIRM' | 'SPARRING' | 'SELECTING_FRAMEWORK' | 'SELECTING_VARIATION' | 'AUDIT_PENDING';

interface FlywheelSession {
    state: FlywheelState;
    rawEntry: string;
    extractedTruth?: string;
    sparringFeedback?: string;
    chosenFramework?: string;
    finalContent?: string;
}

const activeSessions = new Map<number, FlywheelSession>();

export function getFlywheelSession(userId: number): FlywheelSession | undefined {
    return activeSessions.get(userId);
}

export function startFlywheelSession(userId: number, rawEntry: string): void {
    activeSessions.set(userId, { state: 'IDLE', rawEntry });
}

export function updateFlywheelSession(userId: number, session: FlywheelSession): void {
    activeSessions.set(userId, session);
}

export function clearFlywheelSession(userId: number): void {
...
export async function processFlywheelInput(userId: number, text: string): Promise<string> {
    const session = getFlywheelSession(userId);
    if (!session) return "No active flywheel session.";
