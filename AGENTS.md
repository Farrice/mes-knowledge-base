...
- **Front door (v4, 2026-08-10 — `/go` and `/raw-intent-bridge` now run ONE flow, canon in `.agent/workflows/go.md`)**: **translate → second opinion → run in place.** Stage 0 is a model-side Translation Card (anchor · deliverable · audience · **felt standard verbatim** · sharpened line) — NEVER feed the raw dump to a compiler; it routes lexically and mis-routes vision-speech (cold A/B 2026-08-10: an ICP-imagery dump compiled to `/want-vs-need-mapper`). The engines (`codex_operator_preflight.py`, `expert_router.py`, `recommend_stack.py`) run on the **sharpened line only** and are a suggestion you may overrule in one line. Then load the experts and **execute in the same context** — hand off only for genuinely fleet/panel/council-shaped work. Goal spine + one `.agent/missions.jsonl` line at each end; `contract.json`/`portable.md` on request, not as a tax. Sign-off scales with blast radius (T1 states its shape and goes; T2/T3 brief back and wait). Codex adaptations: "Universal Harness" section at the end of `.agent/workflows/go.md` (fleet patterns plan via `codex_dynamic_workflow.py`; real subagents stay approval-gated). `/create` = universal content conductor (outcome contract ≥2 outcomes → context richness → live zeitgeist w/ receipts → purpose routing → proven recipe → gates). Orchestrating multi-step or quality-critical work: load `directives/peak-operation.md` (the operating doctrine — shape of the work, routing table, drift signals, invariants).
