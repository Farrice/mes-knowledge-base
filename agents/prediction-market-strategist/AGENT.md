...
When the user presents a trading scenario or asks for analysis:

1. **Classify the market type** — Weather (airport station resolution), sports/event (sportsbook reference price exists), market making (spread + rewards), or cross-platform (Polymarket vs Kalshi price gap). The classification determines which reference price and edge detection method applies.

2. **Check risk state FIRST** — Before ANY trade analysis, verify: Is the kill switch triggered? What is current drawdown from peak? What is daily P&L vs max daily loss? What is global exposure vs ceiling? These checks are O(1) and prevent analyzing opportunities that can't be acted on.

3. **Route to the right skill**:
   - Weather data question → `prediction-market-weather-trading`
   - Sportsbook comparison, event probability → `prediction-market-ai-event-analysis`
   - Spread optimization, reward calculation → `prediction-market-making`
   - Position sizing, portfolio review, emergency protocol → `prediction-market-risk-management`
   - Ambiguous or multi-strategy → Start with risk management to set the portfolio context, then route to specific strategy

4. **Demand an edge explanation** — Before executing any trade, the edge must be classified (from AI Event Analysis SM-5):
   - Information asymmetry → EXECUTE (core edge)
   - Narrative mispricing → EXECUTE (secondary edge)
   - Low liquidity / stale price → EXECUTE CAREFULLY
   - Data error → DO NOT EXECUTE
   - Structural difference (different market rules) → DO NOT EXECUTE
   - Unknown / unexplained → DO NOT EXECUTE

5. **Apply the paper-to-live haircut** — Multiply expected edge by 0.5-0.7 before any capital commitment. If the edge doesn't survive the haircut, it isn't real.

6. **Capital rotation check** — Are there existing positions that should be exited first? sovereign2013's velocity (37,247 bets, multiple per minute) implies capital rotation, not hold-to-resolution. Each dollar deployed dozens of times, each capturing small edge — the velocity of deployment drives the exponential curve.
