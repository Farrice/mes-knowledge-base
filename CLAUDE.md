...
- **Front doors (2026-09-11, three, both harnesses):** task-shaped → `/autopilot` (mirror · shape check · compile · route · code gates · run · receipt; no sign-off beat, `--preflight` = the old `/go` card) · job-shaped → `/job` (plan beat, lanes, dispatch to seats, packets, MAY END, headless `run`) · deliberation → `/convene`. `/raw-intent-bridge` is a pointer; `/mission` (Mission OS) stays live on the same store; never stack two doors in one message. Operator playbook (doors · seats · headless · Codex-from-here): `directives/two-harness-operator-playbook.md`.
...
> **⚠ LIVING vs RECORD — BINDING (Farrice 2026-08-07).** A filename that **leads with `YYYY-MM-DD-` is a RECORD**: a receipt of one session, never truth, never built on. **No leading date = LIVING**: one per slot, sits at the bucket root, **updated in place**. Reading? Open the project's generated `START-HERE.md` first, then the living docs it lists — open a dated file only for the history of a decision. Writing? Update the living doc; if you're adding a second undated file for the same thing, you're creating the next fourteen-copies mess. **`status: canonical` is RETIRED** — it became an over-powerful attractor where a frozen snapshot outranked newer cumulative work ("everything refers back to that one document instead of the whole"); `superseded_by:` survives only for a doc that was *wrong*, not merely older. Shape is `_active/<arena>/<initiative>/<NN-bucket>/`; the front door is generated by `execution/front_door.py` and is the ONLY file that may claim to be an index. Full standard + the move-tooling traps: `directives/artifact-placement.md`.
