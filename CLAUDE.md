...
- **Per-client CLAUDE.md inheritance** (auto-loads on `cd`): Andrea/Resonance `projects/andrea-dj/` · Jen Santulan `_active/jen-listings/` · Farrice/Parallax `_active/farrice-brand/`. Contract: declare inheritance, one-paragraph identity pointer, Override List, client anti-patterns.
