...
- **Front doors (2026-09-11, three, both harnesses):** task-shaped → `/autopilot` (mirror · shape check · compile · route · code gates · run · receipt; no sign-off beat, `--preflight` = the old `/go` card) · job-shaped → `/job` (plan beat, lanes, dispatch to seats, packets, MAY END, headless `run`) · deliberation → `/convene`. `/raw-intent-bridge` is a pointer; `/mission` (Mission OS) stays live on the same store; never stack two doors in one message.
