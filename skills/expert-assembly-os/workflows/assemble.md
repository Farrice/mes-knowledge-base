...
The deliverable is a single **decision-grade roadmap document** (`outcome.md`), structured exactly per `references/roadmap-schema.md` — never a subset, never reordered:

1.  **Panel (Labeled)**: every seat marked `[Roster]` or `[Bespoke Composite]`, Farrice seated as Function Owner. No unlabeled seats.
2.  **Claims Grounding Table**: present whenever the deliverable contains ANY factual claim — Claim / Source-or-Grounding / VERIFIED-LIKELY-UNCONFIRMED, row by row, never a "per Step X research" abstraction.
3.  **Synthesis**: The Crux (the one real tension), the Net-New Principle (what only emerged from combination — never a single panelist's restated take), Forks for Farrice (genuine either/or with tradeoffs on both sides).
4.  **Roadmap — Three Horizons**: Operational (0–30d), Tactical (1–6mo), Strategic (6–12mo). Every move carries Owner + Action + observable Success Criteria (a number and a date, never "improve X") + Dependencies.
5.  **Composition Ledger**: Seat / Expert / Filled By (Roster or Bespoke) / Coverage / Notes — why this seat exists and how it got filled.
6.  **Next Moves Together**: what was built, what the panel recommends beyond the roadmap, what it didn't get to, and the one Blind Spot it flagged.
7.  **Blind Spots the Panel Flagged**: at least one, with Why / Risk / Mitigation — never a template placeholder.

## Quality Gate

1.  **The Labeling Test**: Is every panelist marked `[Roster]` or `[Bespoke Composite]` — zero ambiguous seats?
2.  **The No-Fabrication Test**: Did every bespoke persona clear `persona_stat_lint.py` with zero flags (no $/%/real-company credentials)?
3.  **The Mastery Floor Test**: Does every bespoke seat carry a receipt with ≥3 source URLs and a CURRENT verdict from `persona_receipt_check.py` — or, if it failed twice, a visible `[MASTERY FLAG: <reason>]` next to that panelist, never a silent seat?
4.  **The Observable-Move Test**: Does every roadmap move have an Owner and a Success Criterion with a number and a date — not "improve X"?
5.  **The Real-Fork Test**: Do the Forks for Farrice present genuine tradeoffs on both sides, not a strawman option next to the obviously-correct one?
6.  **The Grounding Test**: Did the full roadmap pass `grounding_guard.py --task-type Strategy` before delivery?

> **🛡️ Anti-Pattern Check**: Before delivering, review the output against the **Anti-Patterns (Sourced)** section in `genius.md` — fabricated credentials, latent-only personas, silent Mastery Floor failure, unlabeled panels, vague roadmap moves, pinned Opus, rebuild-instead-of-extend. Flag and fix any violation before the roadmap ships.
