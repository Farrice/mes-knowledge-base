...
- `skills/mike-taylor-synthetic-research/`: the deep source behind TRIAGE mode — research design, the three-tier grounding ladder, distribution-vs-individual accuracy discipline, and the multi-question research-budget triage this front door doesn't attempt. Optional depth, not a required step: reach for it when TRIAGE needs grounding-tier discipline (real transcripts exist — `/mt-persona-grounding`), latent-demand discovery (`/mt-latent-demand-mining`), or a full research-budget routing decision (`/mt-synthetic-vs-real-decision`) rather than a 5-minute artifact check.
