---
name: "MOBILE CLAUDE CODE QUICK-START SETUP"
source_prompt: "skills/boris-claude-code/references/prompts/mobile_claude_code_quickstart.md"
...
- **Format**: A completed, verified mobile Claude Code environment — GitHub authenticated, a working repository cloned and navigable, cloud sync confirmed.
- **Components**: Confirmed `/doctor` output showing GitHub authenticated · a cloned repository with `git status` returning clean · confirmed cloud sync (same account on mobile and desktop) · a Quick Command Reference the user can return to.
- **Quality Standard**: The user can run standard git/gh commands from their phone and hand a session off to desktop without re-authenticating or re-cloning.
...
## Quality Gate
- [ ] `gh auth status` confirms authentication before proceeding to repository steps — never assume auth succeeded without checking.
- [ ] The cloned repository's `git status` is verified clean/expected before declaring setup complete.
- [ ] Cloud sync is verified by confirming the same account is logged in on both mobile and desktop, not assumed.
- [ ] Every Common Issue's Fix is tied to its actual Cause — no generic "try restarting" filler.
- [ ] The user is left with a concrete recommended first action, not just a "you're all set" sign-off.
...
**Next step:** Load the full Mobile Orchestration Commander prompt for advanced workflows, multi-agent orchestration, and mobile-first productivity from your phone.

---

*Mobile Claude Code Quick-Start Setup*
*Fast Path to Full Capability*
