...
| `projects/prediction_market_arb/` (polymarket_client.py, market_selector.py, polymarket_ws.py) | — | Farrice's own live project implementing a Polymarket client | VERIFIED to exist; confirms this skill has an active downstream consumer, not cited as a content source for any GP/HK claim |
...
| HK-8 Gambot / vig-stripping mechanics, Pinnacle as sharpest book | **UNCONFIRMED** | No "Gambot" reference, and no explicit Pinnacle-vs-DraftKings/FanDuel margin comparison, appears anywhere in `sovereign-trader-analysis-source.md` or the other raw-source files reviewed this pass. This is standard, well-documented sports-betting market structure (Pinnacle's reputation as the sharp book is public knowledge) but is not independently verified against a specific source in this skill's own research folder. Carried forward as pre-existing skill content — flagged here rather than silently presented as sourced. |
