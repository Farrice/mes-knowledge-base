...
- No other extraction files exist for this expert: `ls extractions/ | grep -i wang` returns only `dan-wang/`. Checked `_active/codex-harvest-2026-06-11/extractions/` for a `wang` match — none found (directory listing empty for this name). Did not need to fall back to the claude-export tarball (`_archive/claude-export-2026-07-01.tar.gz`) because the primary transcript is present, complete, and sufficient to ground every claim below — opening the tarball would not add a Dan Wang source that the primary extraction lacks.
