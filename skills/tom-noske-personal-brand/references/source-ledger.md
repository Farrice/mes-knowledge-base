...
**Search discipline followed**: searched all name fragments (`noske`, `tom noske`, `Tom Noske`) across `extractions/`, `_archive/`, `research_outputs/`, `agents/`, `evolution_store/`. No `_archive/claude-export-2026-07-01.tar.gz` scan was needed — primary transcripts were found directly under `extractions/` and `extractions/_archive/`, so the corpus was not "absent." One video referenced in source #4 ("4 Ingredients for $10K/Month with Small Audience," Video 1 of the dual-video extraction) has NO raw transcript file anywhere on disk outside `.claude/worktrees/` — every claim traceable only to that video is labeled LIKELY, not VERIFIED, below.
...
| Claim | Status | Note |
|---|---|---|
| "generated $3M+ since 2022" | **UNCONFIRMED** | Not found in any of the 3 primary transcripts, the 2 extraction reports, or the research_outputs dossier. Do not cite as a verified fact until matched to a source. |
| "$130K/month average" | **UNCONFIRMED** | Same — no matching figure anywhere in the searched corpus. |
| "$6M cumulative student wins" | **UNCONFIRMED** | Same. |
| "Target $2-4/follower annual earnings" | **UNCONFIRMED (specific range)** | No verbatim "$2-4/follower" figure in any transcript. The *closest* verified analog is Hidden Knowledge #5's "100K followers @ $400K/year... 20x" comparison (below), which implies roughly $4/follower for that one example — a single data point, not a stated universal target. `research_outputs/ai_authority_architect_agents/tom_noske.md` (source #7) independently searched for this exact "$2-4/follower" figure on 2026-06-02 and returned "NO corroborating source... UNCONFIRMED." |
