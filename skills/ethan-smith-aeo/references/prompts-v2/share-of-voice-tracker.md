...
source_prompt: "skills/ethan-smith-aeo/references/prompts/share-of-voice-tracker.md"
...
The deliverable is a single AEO measurement system spec covering all six steps in order. Components, in order:

1. **Tool Selection** — a recommended tool that clears all 4 named evaluation criteria, at a stated cost, explicitly avoiding premium "insights" upsells.
2. **Query Tracking Architecture** — every target query sorted into Tier 1/2/3 by cadence, with 3-5 wording variants per core query.
3. **Share of Voice Calculation** — Mention Rate, Average Position, and Quality-Adjusted SOV computed per surface, plus the aggregate.
4. **Hidden Attribution Detection** — all 4 named signals (branded search correlation, direct traffic anomaly, post-conversion survey, UTM/referral cross-reference) instrumented, with the True Attribution Multiplier calculated from real branded-search/direct/referral data — never assumed without those inputs.
5. **Reporting Dashboard** — the weekly report (6 components) and monthly report (adds 2 more) specified in full.
6. **Conversion Value Weighting** — the LLM traffic value multiplier applied to compute Revenue per LLM Visit and both raw and true (hidden-attribution-adjusted) AEO ROI.

Format: the six sections in the fixed order above, each using the structured block shape from the Execution Framework. Every SOV/ROI figure must show its formula and inputs, never a bare final number.

## Output Skeleton

```
TOOL SELECTION
  Recommended tool: [name] at [$/month]
  Criteria clearance: [4 checks, each pass/fail]

QUERY TRACKING ARCHITECTURE
  TIER 1 (daily): [queries, run frequency]
  TIER 2 (weekly): [queries, run frequency]
  TIER 3 (monthly): [queries, run frequency]
  QUESTION VARIANCE: [3-5 wordings per core query]

SHARE OF VOICE CALCULATION
  [per surface: Mention Rate %, Avg Position #, QA-SOV]
  AGGREGATE: [combined mention rate, combined QA-SOV]

HIDDEN ATTRIBUTION DETECTION
  Branded search correlation: [tracked signal]
  Direct traffic anomaly: [tracked signal]
  Post-conversion survey: [% selecting AI assistant]
  UTM/referral cross-reference: [True Attribution Multiplier, with inputs shown]

REPORTING DASHBOARD
  WEEKLY: [SOV scorecard, top movers, citation source changes, hidden attribution signals, experiment status, recommended actions]
  MONTHLY (adds): [competitor movement, ROI estimate]

CONVERSION VALUE WEIGHTING
  Revenue per LLM visit: [formula + inputs]
  Raw AEO ROI: [formula + inputs]
  True AEO ROI (hidden-attribution-adjusted): [formula + inputs]
```

## Quality Gate

- [ ] Tracking covers 3+ LLM surfaces, not a single surface
- [ ] Query variance is accounted for (3-5 wordings per core query)
- [ ] Answer variance is accounted for (3+ runs per query where tier cadence allows)
- [ ] Hidden attribution is actively instrumented across all 4 named signals, not just referral tracking
- [ ] Post-conversion survey is deployed or explicitly planned with a launch step
- [ ] Every ROI/SOV figure in the output shows its formula and source inputs, not just a bare number
- [ ] Reporting cadence (weekly + monthly) is fully specified, not partially defined
