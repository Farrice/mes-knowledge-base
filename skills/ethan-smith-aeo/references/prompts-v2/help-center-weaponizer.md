---
name: "Help Center Weaponizer"
source_prompt: "skills/ethan-smith-aeo/references/prompts/help-center-weaponizer.md"
...
The deliverable is a single help center weaponization plan covering all six steps in order. Components, in order:

1. **Architecture Audit** — subdomain/subdirectory verdict, cross-linking counts against the 3-5-links target, and content structure inventory.
2. **Tail Question Gap Analysis** — every question found across the 5 named sources (support tickets, sales calls, Reddit/community, competitor help centers, direct LLM queries), each marked covered/not-covered/wrong-answer with an action.
3. **Obscure Question Mining** — new candidate questions generated across the 5 named categories (integration, edge case, migration, limitation, troubleshooting), genuinely zero-competition.
4. **Content Creation Priority Matrix** — every question from Steps 2-3 sorted into P0-P3, with P0 reserved for questions LLMs currently answer incorrectly.
5. **Content Templates** — one filled template per P0/P1 question (title, opening, body outline, cross-links, information gain statement).
6. **Community Q&A Strategy** — a stated go/no-go with implementation specifics if adopted.

Format: the six sections in the fixed order above, each using the structured block shape from the Execution Framework. P0 is reserved exclusively for wrong-answer corrections — it must never be used as a general "urgent" bucket.

## Output Skeleton

```
ARCHITECTURE AUDIT
  Location: [subdomain/subdirectory + migration verdict]
  Cross-linking: [current counts vs. 3-5 target]
  Content structure: [total articles, categories, avg length, freshness %]

TAIL QUESTION GAP ANALYSIS
  [per source: support tickets / sales calls / Reddit / competitors / LLM queries]
    [question → covered status → action]

OBSCURE QUESTION MINING
  [per category: integration / edge case / migration / limitation / troubleshooting]
    [candidate questions, genuinely zero-competition]

CONTENT CREATION PRIORITY MATRIX
  P0 — wrong LLM answers: [list]
  P1 — sole-citation opportunities: [list]
  P2 — thin existing coverage: [list]
  P3 — competitor-only coverage: [list]

CONTENT TEMPLATES
  [per P0/P1 question]
    TITLE: [question]
    OPENING: [direct-answer instruction]
    BODY: [outline]
    CROSS-LINKS: [3-4 targets]
    INFORMATION GAIN: [unique element]

COMMUNITY Q&A STRATEGY
  Adopt: [Y/N + rationale]
  [if Y: implementation specifics]
```

## Quality Gate

- [ ] Subdomain vs. subdirectory issue is explicitly addressed (or confirmed as already subdirectory)
- [ ] Cross-linking audit includes specific link counts measured against the 3-5 target
- [ ] Tail gap analysis covers all 5 sources (tickets, sales, Reddit, LLMs, competitors) — none silently skipped
- [ ] Obscure questions are genuinely obscure (not repackaged head terms)
- [ ] P0 priority is reserved strictly for wrong-answer corrections, distinct from P1 missing-answer opportunities
- [ ] Each content template includes both cross-links AND a specific information gain statement
