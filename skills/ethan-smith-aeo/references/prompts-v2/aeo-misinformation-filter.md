---
name: "AEO Misinformation Filter"
source_prompt: "skills/ethan-smith-aeo/references/prompts/aeo-misinformation-filter.md"
...
This table is calibration reference, not a substitute for running Steps 1-6 on the user's specific claim — use it to sanity-check the final verdict, never to skip the framework.

## Output Contract

The deliverable is a single sequential claim-validation report covering all six steps in order — no step may be skipped or summarized away. Components, in order:

1. **Claim Decomposition** — the exact original claim plus its enumerated sub-claims (factual assertions, causal claims, implied assumptions).
2. **2007 Pattern Filter** — a historical-analogue check across at least the 3 named eras, ending in one pattern verdict.
3. **Evidence Assessment** — which evidence tier the source's proof falls into, plus a reproduction status.
4. **Platform Applause Test** — a single-letter verdict (A/B/C/D) and, if C or D, an estimated half-life.
5. **Convergence Check** — the claim's framing scored against the 4 convergence-thesis checks.
6. **Final Verdict** — one of the 5 named categories, all four upstream findings rolled up, and an explicit recommendation (Adopt / Test before adopting / Ignore / Actively avoid) — with a named experiment design if "Test before adopting" is chosen.

Format: the six sections in the fixed order above, each using the structured block shape from the Execution Framework (not prose paraphrase). A step with no historical analogue or no sub-claims must still say so explicitly rather than being omitted.

## Output Skeleton

```
CLAIM DECOMPOSITION
  ORIGINAL CLAIM: [exact statement]
  SUB-CLAIMS: [enumerated factual / causal / assumption claims]

2007 PATTERN FILTER
  [historical analogue per era, or "no historical analogue"]
  PATTERN VERDICT: [validated / temporary / debunked / no analogue]

EVIDENCE ASSESSMENT
  EVIDENCE LEVEL: [STRONG / MODERATE / WEAK / VERY WEAK / NONE / BIASED]
  REPRODUCTION STATUS: [FULLY REPRODUCED / PARTIALLY REPRODUCED / UNVALIDATED]

PLATFORM APPLAUSE TEST
  VERDICT: [A / B / C / D]
  HALF-LIFE (if C/D): [estimate]

CONVERGENCE CHECK
  [framing check results across the 4 listed checks]

FINAL VERDICT
  CATEGORY: [VALIDATED / PLAUSIBLE / UNVALIDATED / LIKELY FALSE / COMMERCIAL BIAS]
  Evidence level: [from Step 3]
  Reproduction status: [from Step 3]
  Historical pattern: [from Step 2]
  Platform sustainability: [from Step 4]
  Convergence awareness: [from Step 5]
  RECOMMENDATION: [Adopt / Test before adopting / Ignore / Actively avoid]
  [if Test before adopting: named experiment design outline]
```

## Quality Gate

- [ ] Claim is decomposed into testable sub-claims, not left as one blob
- [ ] Historical pattern check covers 3+ eras (or explicitly states "no analogue found")
- [ ] Evidence assessment distinguishes controlled vs. anecdotal evidence, not collapsed into one label
- [ ] Commercial bias is explicitly evaluated, not silently skipped
- [ ] Platform applause test is applied with a named verdict letter
- [ ] Convergence thesis framing is checked against all 4 listed criteria
- [ ] Final verdict names one of the 5 categories AND includes a clear recommendation
