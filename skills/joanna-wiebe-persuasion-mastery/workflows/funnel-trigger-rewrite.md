...
## Input Required
1. **Copy to Rewrite**: The existing landing page, email, ad, sales page, or product description.
2. **Funnel Stage**: ToFu (awareness), MoFu (consideration), or BoFu (decision).
...
If no audit is provided, run a rapid 9-trigger scan (abbreviated version of the Funnel Trigger Audit).

1. Score each trigger as Present/Absent/Miscalibrated for the declared funnel stage.
...
1. **Threat Language Scan**: Any remaining implications that the reader is wrong?
