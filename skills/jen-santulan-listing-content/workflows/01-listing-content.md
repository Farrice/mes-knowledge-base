...
> **Register ladder (2026-08-05, Jen's own verdict — check tier FIRST):** listings **≥$2M** run the "Quiet Flex Elite Advisor" register and slot map from `references/prompts-v2/listing-hook-set.md` — at that tier the First-Time Buyer Permission hook is **forbidden** (rent-vs-mortgage math is factually wrong for the buyer; scar: 5200 Armida) and is replaced by the multigenerational/flexibility hook. Below $1.5M this workflow runs as written, FTHB hook mandatory. Ladder canon: `_active/clients/jen-listings/CLAUDE.md` Override List. For the full URL→sendable-package pipeline (research ledger, caption, send text, compliance gates), use `workflows/listing-package.md` instead. Fair-housing floor on all spoken text: `python3 execution/fair_housing_lint.py check --text "..." --context script`.
