...
| S8 | `extractions/rory-sutherland-cekfxfa3jzu/transcript.txt` + `captions.en.json3` | 75,455-byte transcript; 13,772 words; 4,517-second timestamped captions | Complete 2026 My First Million masterclass. Adds timestamped support for Guidara, Buc-ee’s, Moxy, Uber waiting, Thaler portfolio risk pooling, response-channel choice, action certainty, rejection removal, explicit trade-offs, secondary markets, and advertising archaeology. Full rows: `references/source-ledger-2026-mfm.md`. |
| B1 | `extractions/rory-sutherland-cekfxfa3jzu/alchemy-official-publisher-sample.pdf` + `.txt` | Official Penguin sample; 9,290 extracted words | Orientation only: rules, foreword, and introduction through printed page 25. The full book was not available and was not reviewed. |

`genius.md`'s own CONTENT ASSESSMENT header (line 15) describes its sources as "Long-form podcast interview (~90 min) + Adcom 2026 London keynote (~60 min, ~10,000 words)" for Patterns 1-16, and "three additional Sutherland transcripts mined from the claude.ai export" for Patterns 17-22. No file literally named/tagged "Adcom 2026" or "Adcom keynote" exists anywhere in `extractions/` or the located claude-export conversations — S2 ("How human behaviour regularly defies logic") is the closest match by content (a ~60-minute keynote-style timestamped talk) and is treated as that source. The "three additional" export transcripts are treated as S2/S3/S6 (or S2/S3 plus S4/S5 as a pair), since those are the Sutherland-titled conversations that actually exist in the 2026-07-01 archive per `_active/harness/claude-export/index.json`.
...
| # | Claim | Status | Anchor |
|---|---|---|---|
| 11 | Reverse Benchmarking | VERIFIED | S2 verifies the mechanism and term at timestamp 80:44. S8 verifies the Will Guidara / Eleven Madison Park method at 12:30, coffee and beer specialists at 13:13, Buc-ee’s at 13:46, Moxy at 61:23, and Uber waiting at 30:01. Jumeirah remains unconfirmed and is not used as a v5.0 anchor. |
...
| 13 | Portfolio-Level Asymmetric Betting | VERIFIED (portfolio risk pooling); OPERATIONAL SYNTHESIS (reversibility aid) | S8 at 24:43-27:15 verifies the Thaler division-head example: favorable bets are rejected locally because career downside sits with the sponsor, while the chief executive wants them pooled. S8 at 48:11 verifies the Bezos fat-tail baseball analogy. The “two-way door” attribution remains unconfirmed; v5.0 no longer presents it as Rory doctrine. |
...
**Earlier Pattern 11 case studies**: S8 now verifies Buc-ee's, Moxy Hotels, Uber wait-time, and Guidara/Eleven Madison Park. Jumeirah Departure Lounge remains UNCONFIRMED and was removed from v5 active proof paths. Pattern 16's restaurant-phone-sign example remains at its prior evidence level.
...
The 2026 S8 expansion promotes Guidara, Buc-ee’s, Moxy, Uber waiting, and portfolio risk pooling through exact caption timestamps. It does not blanket-promote unrelated claims. Choice Architecture decoy examples, Subscription Alchemy/Netflix specifics, the Overground/Silverlink story, Jumeirah, technoplasmosis, DoubleTree/AO/Dishoom, and Hotels.com remain at their prior evidence levels. The full text of *Alchemy* remains unavailable; B1 is a publisher sample only.
