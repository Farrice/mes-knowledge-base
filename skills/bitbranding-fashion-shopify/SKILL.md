...
5. **Stack**: pair Step 4 with `/posters` for hero generation; pair Step 1 with Oren for brand positioning if not yet locked
...
Single MES 3.0 extraction from one BitBranding YouTube tutorial (Represent collection-page rebuild on Horizon). 6 genius patterns, 5 hidden-knowledge insights, 3 exemplars (Represent, Christian's rebuild, anti-exemplar default theme), 5 signature moves, 6-criterion quality rubric.

**Source limitation**: Single-tutorial extraction. Christian's full methodology likely spans 5-10 videos on his channel. If this skill underdelivers, route to a multi-source extraction expansion (collect 4-5 more BitBranding videos, run forge).

**Honest scope**: This skill captures Christian's collection-page expertise. It does NOT cover product page (separate video), cart/checkout (separate video), homepage, navigation, or email/SMS post-purchase. Those are gaps for future expansion.
