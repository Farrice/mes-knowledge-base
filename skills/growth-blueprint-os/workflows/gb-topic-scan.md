...
- **Research-control owner:** read `skills/kallaway-ai-content-engine/genius.md` Patterns 8–12 and `workflows/ai-topic-mining-engine.md`. Growth Blueprint consumes those judgments; it does not redefine them.
- **Maturity declaration:** use pack `data_maturity_state` when declared. If `UNDECLARED`, collect the owned post count and map it to `COLD_START` (<10), `HYBRID` (10–19), or `OWNED_LEARNING` (20+). Public competitor rows stay `PUBLIC_PROXY` in every state.
...
Take `ranked_videos` (outlier-score-sorted, cap 50; the pack normalizes for channel size — `outlier_multiplier = N× that channel's normal`, define it inline in the artifact so no reader has to look it up). First assign every row `TOPIC_COHORT`, `FORMAT_ONLY`, or `EXCLUDE`: topic evidence must be same-niche and comparable-scale; cross-niche or celebrity-scale rows may transfer format, hooks, editing, or visual craft only. Respect producer `signal_hygiene`: `REJECT` stays excluded, `REVIEW` stays visibly uncertain, and only `PASS` clears the 2% engagement control. Screen every row with a **visible verdict, never a silent drop**: ✓ qualifies for the avatar / ✗ excluded, with a two-or-three-word reason on every ✗ ("wrong avatar," "too broad," "off-niche," "person-not-topic," "low engagement"). ✗ rows STAY in the table, struck through, in rank position — the operator overrules any call, and a beginner learns as much from the struck rows as the kept ones. Backfill from next-ranked candidates so ~50 rows carry ✓. Itemize exclusions by category with counts in the stage report (the free-triage report shape, adopted).
...
Tag each topic bucket with its ring. Check the shape of what's winning: winners clustering at Rings 4–5 means the niche's data skews broad and the operator must consciously *protect* their narrow picks rather than chase "what's working"; winners at Rings 2–3 confirm the narrow aim. Then the **conversion column (ours):** every bucket → the offer it feeds → funnel role → the "working =" threshold, pulled from the bullseye revenue overlay — so the table asks "which of these 50 could serve the buyer?", not pretend public reach proves a purchase. Rank evidence by email conversions or qualified leads > relevant followers gained > views. A bucket with no offer linkage or owned outcome remains honestly reach-only.
