...
| Field | Contract |
|---|---|
| Source evidence | Eleven packages under `extractions/video-context/`: `oRYfJ_yxz6M`, `B9l9TRhu5Vw`, `ImzoNTrgvFg`, `cuVyTmbOZjk`, `bqzd0h0gmU0`, `onQoYdxWXdA`, `a7VjpIqq8Xk`, `SDHKQbKC7gA`, `7pCEsr-0KIw`, `1q__Vs2JqbI`, `1ilMGCxJBQY`. Exact presence and uncertainty remain governed by `source-evidence-map.md` and `source-ledger.md`. |
| Objective | Make the existing Kallaway suite operate as one source-backed content OS from qualified category authority and decisive positioning through production, retention, articulation, batch learning, and monetization. |
| Components | `kallaway-content-operating-system`, `workflows/micro-fame-authority-density.md`, bounded `luke-iha-insight-vectors` authority-transfer support, `kallaway-content-system`, `kallaway-content-psychology`, `kallaway-ai-content-engine`, `skills/kallaway-ai-content-engine/workflows/trend-hook-radar.md`, `execution/kallaway_trend_hook_radar.py`, `kallaway-addictive-storytelling`, `kallaway-audience-obsession`, `kallaway-word-mastery`, `kallaway-social-commerce`, `agents/kallaway`, `execution/video_context_ledger.py`, command/workflow routers, validation scripts. |
| Step order | intent lock -> evidence selection -> authority diagnosis and decisive contrast when relevant -> authority-transfer depth test -> compliant signal radar when needed -> lane classification -> component chain -> handoffs -> first artifact -> evidence-floor review -> validation -> next use route. |
...
| Outputs | Route trace, source evidence summary, authority diagnosis or contrast brief when relevant, one grounded perspective shift or explicit no-shift finding for narrow authority content, signal/outlier receipt when relevant, component handoffs, first usable artifact, evidence-floor review, validation checklist, next command sequence. |
...
| Behavior-changing proof | `references/authority-density-behavior-proof.md` applies the companion to a frozen synthetic health-performance case and proves it rejects a vanity-reach false winner, preserves lower-reach commercial signals, and holds chaos promotion until four fair reps. |
...
| Reuse hook | Invoke `/kallaway-content-os` whenever a request asks for the full Kallaway content system, micro-fame, authority density, decisive personal-brand positioning, 3-2-1 authority planning, or a four-rep review. Invoke `/kallaway-trend-hook-engine` when the request asks for Kallaway hook trend analysis, a Sandcastles alternative, social outlier scoring, or compliant trend radar behavior. |
