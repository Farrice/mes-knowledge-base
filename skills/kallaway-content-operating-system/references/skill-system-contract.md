...
| Components | `kallaway-content-operating-system`, `workflows/micro-fame-authority-density.md`, `kallaway-content-system`, `kallaway-content-psychology`, `kallaway-ai-content-engine`, `skills/kallaway-ai-content-engine/workflows/trend-hook-radar.md`, `execution/kallaway_trend_hook_radar.py`, `kallaway-addictive-storytelling`, `kallaway-audience-obsession`, `kallaway-word-mastery`, `kallaway-social-commerce`, `agents/kallaway`, `execution/video_context_ledger.py`, command/workflow routers, validation scripts. |
| Step order | intent lock -> evidence selection -> authority diagnosis and decisive contrast when relevant -> compliant signal radar when needed -> lane classification -> component chain -> handoffs -> first artifact -> evidence-floor review -> validation -> next use route. |
...
| Outputs | Route trace, source evidence summary, authority diagnosis or contrast brief when relevant, signal/outlier receipt when relevant, component handoffs, first usable artifact, evidence-floor review, validation checklist, next command sequence. |
