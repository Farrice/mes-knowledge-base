...
|---|---|---|
| 9 of the 10 video packages listed in `source-evidence-map.md`'s Source Packages table (`oRYfJ_yxz6M`, `ImzoNTrgvFg`, `cuVyTmbOZjk`, `bqzd0h0gmU0`, `onQoYdxWXdA`, `B9l9TRhu5Vw`, `SDHKQbKC7gA`, `7pCEsr-0KIw`, `1q__Vs2JqbI`) exist under `extractions/video-context/<id>/` | `skills/kallaway-content-operating-system/references/source-evidence-map.md`, Source Packages table | Ran `ls extractions/video-context/`: only 6 directories exist total (`-WCNwxz3uoM`, `3iR3kHxCwfo`, `FD53kEpLh9c`, `Zc4E_K48v48`, `a7VjpIqq8Xk`, `ohKt066uFhg`), and of those, only `a7VjpIqq8Xk` matches an ID claimed in the table. The evidence-row counts quoted for the other nine (e.g. "1886 spoken rows, 20 visual rows") cannot be re-verified because the underlying files do not exist at the stated path. One of the nine (`B9l9TRhu5Vw`) is recoverable under a different real path: `extractions/kallaway-content-system/transcript.txt` + `B9l9TRhu5Vw.en-orig.vtt` (398,725 bytes). The remaining eight have no located transcript anywhere in this repository as of this pass. This is a pre-existing gap in a file this repair batch did not touch (the `source_ledger` check already passes on `source-evidence-map.md`'s presence, not its accuracy) — flagged here rather than silently worked around, per the "false absent/present claims" rule in the worker envelope. |
