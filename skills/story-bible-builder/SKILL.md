...
Many users pair the bible with a video prompt director skill (like `cinema-worldbuilder-pro` or similar). In this pairing, the director skill handles the cinematography grammar, mode selection, frame composition, and prompt syntax. The bible provides the identity, voice, movement, aesthetic era locks, and canon that the director skill can't get from a reference image alone.
...
1. If a video prompt director skill is also active in the session (e.g., `cinema-worldbuilder-pro` or similar), the director skill handles cinematography, mode selection, frame composition, and prompt syntax
...
Ship the file with the YAML frontmatter (name + pushy description) and the full canon body. Save to `projects/<project>/bible/[working-title-slug].md` (or `_active/<client>/bible/` for client worlds) and present.
