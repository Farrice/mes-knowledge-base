...
source_prompt: "skills/andy-lo-premium-websites/references/prompts/14-deploy-and-export.md"
...
## Output Contract
- A deployment path chosen from the four options (Netlify direct/GitHub, Vercel, Firebase Studio export, generic static host), matched to {{HOST}}
- Custom domain setup steps (conditional on {{DOMAIN}} being set)
- Complete OG/Twitter meta tag block, using the hero starting frame as the OG image
- A completed production verification checklist

## Output Skeleton
```
DEPLOYMENT PATH
Chosen: [Netlify direct / Netlify+GitHub / Vercel / Firebase Studio export / generic static host]
Why: [matched to {{HOST}} and {{BUILD_TOOL}}]
Steps: [numbered steps for the chosen path]

CUSTOM DOMAIN SETUP (if {{DOMAIN}} set)
DNS records: [A record + CNAME targets]
HTTPS: [confirm automatic/enabled]
Propagation note: [expected wait window]

SOCIAL SHARING META BLOCK
Title: {{SITE_TITLE}}
Description: {{META_DESCRIPTION}}
OG image source: [hero starting frame]
[confirm OG + Twitter tag sets both present]

PRODUCTION VERIFICATION CHECKLIST RESULT
- [ ] Production URL loads correctly
- [ ] All pages render as expected
- [ ] Animations play smoothly
- [ ] CMS content loads (if integrated)
...
## Quality Gate
- [ ] The deployment path chosen matches {{HOST}}, not defaulted to Netlify regardless of preference
- [ ] Domain setup steps only appear when {{DOMAIN}} is actually set — otherwise explicitly marked skipped
- [ ] The OG image is sourced from the hero starting frame, not a generic or unrelated image
- [ ] All ten production verification checks are run and pass, including the explicit Lighthouse score threshold
- [ ] Console is confirmed free of errors before the deployment is marked complete
