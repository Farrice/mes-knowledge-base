...
source_prompt: "skills/andy-lo-premium-websites/references/prompts/07-supabase-asset-pipeline.md"
...
## Output Contract
- A Supabase bucket named `{{PROJECT_NAME}}-assets`, set to public
- A folder structure separating frames (by section), images (by category), and raw videos
- Sequentially named frame uploads following the `frame_NNN.webp` convention
- A JSON manifest mapping every asset category to its public URL(s)
- A completed performance verification checklist
...
FOLDER STRUCTURE
frames/hero, frames/section-N... | images/products, images/backgrounds, images/branding | videos/raw
[confirm present, one line per folder]

UPLOAD LOG
- [folder]: [frame count] frames uploaded, sequential naming confirmed
[repeat per animated section]

FRAME URL MANIFEST (JSON)
{
  "[category]_frames": ["[public URL pattern]", "..."],
  "images": { "[asset name]": "[public URL]" }
}
...
## Quality Gate
- [ ] Bucket is confirmed public before any upload begins
- [ ] Frame files follow the sequential naming convention with zero gaps or duplicate indices
- [ ] The JSON manifest includes every asset category referenced in the project (no orphaned folders missing from the manifest)
- [ ] All four performance verification checks are run and pass, not assumed
