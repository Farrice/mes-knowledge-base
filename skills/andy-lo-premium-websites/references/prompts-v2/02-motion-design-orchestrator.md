...
source_prompt: "skills/andy-lo-premium-websites/references/prompts/02-motion-design-orchestrator.md"
...
## Output Contract
- Google Flow setup steps (mode selection, upload order, prompt placement)
- One motion prompt specifying pacing, camera movement, element behavior, atmospheric effects, and emotional arc
- Quality settings: resolution, duration (seconds), style
- A WebP conversion protocol with exact settings (resolution, frame rate, quality value, loop)
- A frame storage plan: bucket destination, naming convention, URL list placeholder

## Output Skeleton
```
GOOGLE FLOW SETUP
1. [tool/mode selection steps]
2. [upload order: starting frame, ending frame]
3. [where the motion prompt gets pasted]

MOTION PROMPT
Pacing: [slow / natural / energetic — with rationale]
Camera movement: [static / pan / zoom / orbit / parallax]
Element behavior: [what moves — background, foreground, particles, lighting]
Atmospheric effects: [lighting shifts, color temperature, depth-of-field]
Emotional arc: [feeling the animation builds toward]

QUALITY SETTINGS
Resolution: [original, matched to source frames]
Duration: [seconds] seconds
Style: [cinematic / natural / dynamic]

WEBP CONVERSION PROTOCOL
- Resolution: [original — no downscale]
- Frame rate: [closest to native FPS]
- Quality: [value] — [one-line rationale for this value]
- Loop: [forever/enabled]

FRAME STORAGE
- Destination: [bucket/path]
- Naming convention: [pattern]
- Frame URL list: [placeholder for URLs, populated after upload]
```

## Quality Gate
- [ ] The motion prompt names pacing, camera movement, element behavior, atmospheric effects, and emotional arc — all five present, none generic
- [ ] No random or chaotic motion described (no "random zooming," no ungrounded particle effects) — every movement is tied to the visual arc
- [ ] WebP settings specify an explicit quality value and frame-rate rule, with a one-line rationale for each
- [ ] Frame storage plan includes a naming convention that supports sequential ordering
- [ ] The transition reads as depth (elements at different distances moving at different speeds), not a flat crossfade
