...
| "Patterns from claude.ai export — Nick Saraev conversations (2026-07-01)" subsection (pre-existing, untouched content) | claimed: 2026-07-01 claude.ai export, "six transcript-grounded extractions" | not locatable | UNCONFIRMED — searched `extractions/` (all 4 Saraev folders = 1 transcript, terms absent) and `_active/harness/claude-export/` (no Saraev-named file); flagged inline with a provenance note, content preserved per additive-first boundary |
