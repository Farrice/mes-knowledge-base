...
|---|---|---|
| "I specifically didn't want to make this fully autonomous..." | genius.md — How to Use / Operating Principles / Anti-Patterns | `_active/codex-harvest-2026-06-11/extractions/video-context/_3SEUgRCXX0/video-context-ledger.md` rows 00:01:55.600–00:01:59.680 |
...
| Video metadata (title, channel, publish date, URL, duration) | genius.md — multiple sections | `_active/codex-harvest-2026-06-11/extractions/video-context/_3SEUgRCXX0/analysis.md` lines 5-9 |
...
| Live-path discrepancy (`extractions/video-context/_3SEUgRCXX0/` referenced but absent at repo root) | references/source-ledger.md | Verified absent via `find . -iname "*3SEUgRCXX0*"` (repo root scope) — zero hits under `extractions/`; present under `_active/codex-harvest-2026-06-11/extractions/...` (byte sizes recorded via `wc -c`) |
...
- The video-context package the skill's own note points to is NOT absent overall — it exists at a different path (`_active/codex-harvest-2026-06-11/...`), fully read, byte sizes recorded above. This distinction matters: the source is real and was used, only the note's stated path is stale.
