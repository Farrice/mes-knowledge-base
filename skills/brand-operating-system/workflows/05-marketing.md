...
Phase E produces 9 files across two directories:

- **`03-marketing/01-content-pillars.md`** — 5-7 pillars, each with name, definition, 3-5 example posts, a numeric proportion of total content (proportions summing to 100%), default channel, default voice pattern.
- **`03-marketing/02-hook-library.md`** — ≥30 hook patterns, each tagged with pillar + ICP profile + channel.
- **`03-marketing/{03-channel-architecture, 04-curation-mechanics, 05-crisis-comms, 06-why-gate-mechanics, 07-funnel, 08-offer-card}.md`** — 6 operational docs. Crisis comms needs ≥3 named scenarios with response templates; why-gate needs the actual application question plus adjudication criteria (not a placeholder question); funnel needs a named hand-off at every step, not "convert."
- **`05-ops/{03-drift-signals, 04-success-metrics, 05-exit-interview-protocol}.md`** — verbatim ports from the founder's canonical inputs. Drift-signals must include the readback ritual, success-metrics must include first-cycle target + horizon metrics + kill condition, exit-interview needs ≥6 questions.
