...
- **Ceiling:** 12 words on the image. *"{{SPINE_FRAME}}."* + *"{{CITY}}. June 2026."* is the maximum stack.
...
- Lower-left overlay: GT Sectra headline 32px — *"{{CITY}}. The first event is June 2026."*
...
### Do
- Use stories for daily texture: *one photo, one question*. {{FOUNDER_NAME}}-led voice memos, behind-the-scenes peeks at music curation, ICP recognition prompts.
...
### Distribution rules
- Flyers go up only at venues whose own aesthetic doesn't cannibalize {{BRAND_NAME}} — independent record stores, daytime cafés, dance studios, bookshops.
...
- **Front:**
  ...
  - Lower-right: hand-script signature of {{FOUNDER_NAME}} (the only place the hand-script appears on a public-facing asset for Event #1; later events may include a couple's name in hand-script per {{FOUNDER_NAME}}'s curation)
- **Back:**
  ...
  - Center-right: a single sentence in `body-md`, ink-700: *"You were in the room when [date]."* — handwritten or typeset depending on whether it's a pre-event ticket (typeset) or a post-event keepsake (hand-written by {{FOUNDER_NAME}} or curator)
...
- Mass-produce. The ticket carries {{FOUNDER_NAME}}'s care; the print run reflects that.
...
- **Hero block (middle 1/2):**
  ...
  - Caption beneath in `caption` (13px) ink-700: *"Photographed at Event #1, {{CITY}}, June 2026."*
- **Body block (lower 1/4):**
  - 3 columns
    - **What it is:** *"{{ONE_LINER}} {{SPINE_FRAME}}."* (body-md)
    - **The mechanic:** *"Daytime. Sober. Curated music. Curated crowd."* (body-md)
    - **Press contact:** {{FOUNDER_NAME}} + Farrice contact details, in body-sm
...
- Use Word's default margins and stylings. This is designed in InDesign, Figma, or Pages with the {{BRAND_NAME}} grid honored.

---

## 9. Web Hero — {{BRAND_NAME}} Site (when it exists, post-Event #1)
...
- *"{{SPINE_FRAME}}."*
- *"A daytime, sober dance party in {{CITY}}."*
...
- `brand-library-entry.md` — the entry that feeds external `knowledge/design-libraries/brands/{{BRAND_NAME_LOWER}}/`
