...
*Locked Canva templates and brand-system setup so {{FOUNDER_NAME}}'s daily design workflow stays on-brand without rebuilding the system every time. One-time setup: ~45 minutes. After that: open template, swap photo, swap copy, ship.*
...
Canva is the daily workhorse for {{FOUNDER_NAME}}'s design-execution layer. Specifically: IG feed posts, IG stories, flyers, ticket layouts, email header banners, sponsor decks, and quick social graphics. Canva is **not** the right tool for: the press one-sheeter (use a designer + InDesign), the printed event ticket on real cardstock (use a print partner), or the website (use a real web stack, not Canva Sites).

The job of this spec: pre-load every brand variable into Canva *once* — fonts, colors, photo treatment presets, type sizes, layout grids — so that any new design {{FOUNDER_NAME}} makes inherits the system without her having to remember every token. Pre-load the system; design from the system; never improvise the system mid-asset.
...
1. **GT Sectra** (license required — purchase at grilli.type)
   ...
   - This is the heat serif. Do NOT upload heavier weights — {{BRAND_NAME}} never uses 700+ on Sectra.
...
3. **Caveat** (free — Google Fonts)
   ...
   - This is the rare hand-script. Used **only** for {{FOUNDER_NAME}}'s signature on a thank-you note. NOT for body, NOT for headlines, NOT for navigation, NOT for event tickets.
...
|---|---|---|
| {{BRAND_NAME}} Cream 100 | `#F5EFE3` | Background canvas |
| {{BRAND_NAME}} Cream 50 | `#FBF7F0` | Surface layer (one tonal step lighter) |
| {{BRAND_NAME}} Midnight 900 | `#0F1A2E` | Body type / headline / event badge fill |
| {{BRAND_NAME}} Ink 900 | `#1A1814` | Body type alt / fine detail (slightly warm black) |
| {{BRAND_NAME}} Ink 700 | `#3A332B` | Long-body text |
| {{BRAND_NAME}} Terracotta 600 | `#B8492E` | Primary CTA / single accent per layout |
| {{BRAND_NAME}} Terracotta 700 | `#9A3A22` | Hover / pressed state for CTAs |
| {{BRAND_NAME}} Slate 500 | `#7A6F62` | Captions, metadata, dates |
| {{BRAND_NAME}} Gold 600 | `#8C6526` | Tertiary accent (rare — couples' names in story-capture only) |
| {{BRAND_NAME}} Border Hairline | `#E1D6C2` | 1px dividers, input borders |
...
- Pure white (`#FFFFFF`) — {{BRAND_NAME}} uses Cream-50, not white
- Pure black (`#000000`) — {{BRAND_NAME}} uses Ink-900, not pure black
...
Canva applies filters/effects/adjustments per-image. The {{BRAND_NAME}} discipline is: **no filters, no effects, daylight color preserved**. The "preset" for {{BRAND_NAME}} photography is *no preset.*

### The {{BRAND_NAME}} Photo Treatment
...
Canva doesn't currently support saving "no-effects" as a named preset. Alternative: create a **Brand Template** (Step 5) with the photo treatment baked in. {{FOUNDER_NAME}} pulls from the template instead of starting blank.
...
- **Tone**: paste the {{BRAND_NAME}} one-paragraph voice description (from `00-foundation/03-voice-document.md` §1)
...
**Layout**:
- Wordmark "{{BRAND_NAME}}" top-left, GT Sectra 24pt, Cream-100 (against photo) or Midnight-900 (against cream).
...
- CTA at bottom: Inter Regular 14pt, Terracotta-600, format: `Application opens June 1 · {{BRAND_NAME_LOWER}}.[domain]/apply`.

**Photo placeholder**: a real daytime room photo (post-Event-#1) or the {{FOUNDER_NAME}}-at-decks daytime portrait (pre-Event-#1).
...
**Layout**:
- Cream-50 background.
- Wordmark "{{BRAND_NAME}}" left-aligned, GT Sectra 32pt, Midnight-900.
...
If {{FOUNDER_NAME}} is doing this from scratch in a single session:
...
The end-state {{FOUNDER_NAME}} operates in:
...
**{{FOUNDER_NAME}} never starts from a blank Canva canvas for any of these surfaces.** That's the whole point of the setup.
...
**Failure: A photo looks "flat" after import — {{FOUNDER_NAME}} wants to brighten it.**
...
**Failure: {{FOUNDER_NAME}}'s collaborator opens a template and sees default Canva fonts (not GT Sectra).**
*Fix*: Brand Kit access requires Pro+ tier. Confirm the collaborator is added to the {{BRAND_NAME}} Brand Kit (not just shared on a template).
