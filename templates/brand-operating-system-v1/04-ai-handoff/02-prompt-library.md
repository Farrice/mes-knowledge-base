...
*22 ready-to-paste prompts for common {{BRAND_NAME}} tasks. Copy a prompt block, paste into Claude/ChatGPT (with the AI Brain Master loaded), fill in the bracket variables, send. Each prompt produces an on-brand draft on first try.*
...
```
TASK: Draft an Instagram feed post caption for {{BRAND_NAME}}.

PROFILE TARGET: [umbrella / {{ICP_PROFILE_1_NAME}} / {{ICP_PROFILE_2_NAME}} / {{ICP_PROFILE_3_NAME}}]
...
**Expected output**: a 100-180 word caption with a sharp hook, a specific moment in the middle, a single-line close. No hashtags. Reads like {{FOUNDER_NAME}} wrote it.

**Calibration notes**: if the hook reads as a marketing question ("Are you tired of swiping?"), reject and rewrite. The hook must be something {{ICP_PROFILE_1_NAME}}/{{ICP_PROFILE_2_NAME}}/{{ICP_PROFILE_3_NAME}} would actually say to a friend.
...
```
TASK: Draft a Reel storyboard for {{BRAND_NAME}}.

PROFILE TARGET: [umbrella / {{ICP_PROFILE_1_NAME}} / {{ICP_PROFILE_2_NAME}} / {{ICP_PROFILE_3_NAME}}]
DURATION: [30s / 45s / 60s]
HOOK TYPE: [show-first scene / out-loud-asking voiceover / {{FOUNDER_NAME}}-on-camera direct address]
...
- The audio is either {{FOUNDER_NAME}}'s VO, ambient room sound, or a music cue from a curated track (never trending TikTok audio).
...
```
TASK: Draft a Story sequence for {{BRAND_NAME}} — [N] frames.
...
```
TASK: Draft an editorial newsletter issue for {{BRAND_NAME}}.
...
Voice: {{FOUNDER_NAME}}'s letter from a friend. No HTML banner. No social icons. Plain-letter feel.
...
```
TASK: Draft an anniversary post for {{BRAND_NAME}}.

MILESTONE: [name it specifically — "one year since Event #1" / "tenth {{BRAND_NAME}}" / "fifth couple to come back partnered"]
...
TONE: warm, grounded, no self-congratulation. The room did the work, {{FOUNDER_NAME}} is grateful.
...
**When to use**: a behind-the-scenes piece showing {{FOUNDER_NAME}} at the decks (rehearsing, building a set, reading the room).

```
TASK: Draft a behind-the-DJ-booth piece for {{BRAND_NAME}}.

SURFACE: [IG caption / Story / email newsletter]
SCENE: [name what we're showing — {{FOUNDER_NAME}} building a setlist / {{FOUNDER_NAME}} at the decks during Event #N / {{FOUNDER_NAME}} testing a track that almost didn't make the cut]
...
LENGTH: matches the surface.
VOICE: first-person {{FOUNDER_NAME}}. Personal but not confessional. Specific over general.

PATTERN: show-first opener mandatory. Mechanic-as-sentence for the rule. Crystallized phrases for the close.

Voice test: does this read like {{FOUNDER_NAME}} telling a friend over coffee about a moment she's still thinking about?
...
```
TASK: Identify and frame a manifesto excerpt for {{BRAND_NAME}}.
...
**When to use**: a stranger DMs {{FOUNDER_NAME}} on IG with interest. Decide if they're hell-yes, polite-yes, tourist, or no.

```
TASK: Triage this {{BRAND_NAME}} DM.
...
3. Recommended next action:
   - "Reply with X" (with the actual reply drafted in {{FOUNDER_NAME}}'s voice)
   - "No reply needed" (with why)
   - "Flag for {{FOUNDER_NAME}} to read personally" (if it's edge-case)
...
```
TASK: Test this proposal against {{BRAND_NAME}}'s 12 Non-Negotiables.
...
1. Run the proposal against each of the 12 Non-Negotiables (per `00-foundation/05-non-negotiables.md`):
   ...
   5. {{CITY}}-first  6. Founder-curated music  7. Stories over metrics
...
4. If REJECT, draft a 80-120 word decline in {{FOUNDER_NAME}}'s voice — warm, specific, doesn't
...
**When to use**: {{FOUNDER_NAME}} is considering a decision (a venue, a guest DJ, a sponsor, a content choice) and wants to check if it's drifting from the spine.
...
DECISION:
"""
[describe the decision {{FOUNDER_NAME}} is considering]
"""
...
Be honest. {{FOUNDER_NAME}} would rather hear "this drifts" early than realize it after the room
has shipped.
```
...
```
TASK: Voice-check this {{BRAND_NAME}} draft against the standards in
...
2. PROFILE FIT — does the draft match the named target ({{ICP_PROFILE_1_NAME}}/{{ICP_PROFILE_2_NAME}}/{{ICP_PROFILE_3_NAME}}/umbrella)? pass/fail.
3. VOICE MATCH — could {{FOUNDER_NAME}} say this to a friend over coffee? pass/fail.
...
```
TASK: Draft {{FOUNDER_NAME}}'s reply to this inbound venue email for {{BRAND_NAME}}.
...
CONTEXT: {{FOUNDER_NAME}} wants the venue for [event date], [headcount], [3-hour 2-5pm window].

OUTPUT: a 100-200 word reply in {{FOUNDER_NAME}}'s voice. Calibrate against
...
Sign-off: {{FOUNDER_NAME}}'s first name only.
```
...
```
TASK: Draft {{FOUNDER_NAME}}'s reply to this inbound press email for {{BRAND_NAME}}.

OUTLET: [name + type — local {{CITY}} paper, podcast, national mag]
THEIR ASK:
"""
...
2. Set expectations: {{FOUNDER_NAME}} is selective about press; she does interviews, not soundbites.
...
PROFILE THEY MATCH: [{{ICP_PROFILE_1_NAME}} / {{ICP_PROFILE_2_NAME}} / {{ICP_PROFILE_3_NAME}} / umbrella]
...
5. Closes with {{FOUNDER_NAME}}'s signature.
...
STRUCTURE:
1. Subject: "Not this round." OR "From {{FOUNDER_NAME}}, on Event #N."
2. Body: warm decline, specific reason (without insulting), an honest door open or closed.
3. Sign-off: {{FOUNDER_NAME}}'s first name only.
...
```
TASK: Generate a daytime-locked Midjourney/Sora prompt for {{BRAND_NAME}}.

SCENE: [describe what we want — "dance floor in real afternoon light," "hands at a
water station," "{{FOUNDER_NAME}}-style DJ at decks daytime," "Pilsen sunset corner," etc.]

CONSTRAINTS (mandatory):
- Real daytime light (specifically: south-facing window, 2pm afternoon, overcast {{CITY}},
...
- Tight crop on hands, gestures, the side of a body. NO identifiable faces (faces are
  forbidden in AI imagery for public {{BRAND_NAME}} use).
...
AESTHETIC: editorial broadsheet left in actual sunlight ({{BRAND_NAME}}'s anchor aesthetic).

OUTPUT: 6-10 reference categories, each with 1-3 specific examples {{FOUNDER_NAME}}/Farrice can
search for:
...
3. COLOR references — specific images / objects / spaces that use the cream + midnight +
   terracotta palette in the way {{BRAND_NAME}} uses it.
...
**When to use**: {{FOUNDER_NAME}} is describing a song to play in a setlist or content piece and wants to surface specific track recommendations.

```
TASK: Name 3-5 specific tracks that match this description for {{BRAND_NAME}}.

DESCRIPTION:
"""
[{{FOUNDER_NAME}} describes the song — "the moment a song bends the room toward someone you
...
CONTEXT: {{BRAND_NAME}}'s curatorial spine — {{FOUNDER_NAME}}-curated, beat-to-beat, no genre lock.
...
- BPM if known (helps {{FOUNDER_NAME}} pace the set)
...
```
TASK: Generate 3 hook variants for this {{BRAND_NAME}} post.

POST PURPOSE: [name it]
PROFILE TARGET: [{{ICP_PROFILE_1_NAME}} / {{ICP_PROFILE_2_NAME}} / {{ICP_PROFILE_3_NAME}} / umbrella]
...
VOICE: {{FOUNDER_NAME}} on a good day. No marketing questions. No TED-talk openers.
```
...
### Prompt 20 — Pre-Event Briefing ({{FOUNDER_NAME}}'s Self-Prep)

**When to use**: 48 hours before an event. {{FOUNDER_NAME}} wants a tight pre-event briefing covering the room, the set, the protocol, the contingencies.

```
TASK: Generate {{FOUNDER_NAME}}'s pre-event briefing for [Event #N].

OUTPUT:
1. THE ROOM — confirmed attendees count, profile breakdown (P1/P2/P3 mix), any specific
   people {{FOUNDER_NAME}} wants to introduce to specific others.
...
5. ANDREA'S NOTE — one sentence {{FOUNDER_NAME}} reads to herself before doors open. The reminder
   of why.
...
**When to use**: ~7 days after an event. {{FOUNDER_NAME}} wants to capture stories from attendees while they're fresh.
...
```
TASK: Run a funnel health check for {{BRAND_NAME}}.
...
3. RESONANCE-SPECIFIC: are we optimizing for couples-formed or for ticket-count?
   (Per `03-marketing/07-funnel.md`, {{BRAND_NAME}} breaks the standard playbook.)
...
When a new task type emerges in {{FOUNDER_NAME}}'s workflow that doesn't fit a prompt above:
